...
REM InlongSlicer build script for Windows. Run with -h for the options.
...
call :add_arg build_slicer bool s slicer "Build InlongSlicer"
...
call :add_arg install_slicer bool i install "Install into the build tree's InlongSlicer folder"
...
REM install(TARGETS InlongSlicer_app_gui) carries no OPTIONAL, so installing a tree
REM whose executable was never built fails. Say so before the build starts.
if "%build_slicer%" == "ON" if "%install_slicer%" == "ON" if not "%slicer_target%" == "" if /I not "%slicer_target%" == "InlongSlicer_app_gui" (
...
if "%build_deps%" == "ON" (
    ...
    if not "%no_configure%" == "ON" (
        call :print_and_run cmake -S deps -B "!DEP_TREE!" -G "%generator%" %gen_args% -DCMAKE_BUILD_TYPE=%build_type% !deps_args! %INLONG_DEPS_CMAKE_ARGS%
        %error_check%
    )
...
if "%pack_deps%" == "ON" (
    ...
    echo Packing the dependencies: InlongSlicer_dep_win-!dep_variant!_!build_date!.zip
    ...
    call :print_and_run !zipper! InlongSlicer_dep_win-!dep_variant!_!build_date!.zip InlongSlicer_dep
    %error_check%
    REM endlocal is about to discard the name, so carry the path out with it.
    for %%z in ("!DEP_TREE_PACK!\InlongSlicer_dep_win-!dep_variant!_!build_date!.zip") do endlocal & set "bundle=%%~fz"
)

if "%build_slicer%" == "ON" (
    set "stage=s"
    echo Building InlongSlicer...
    ...
    REM A later -DSLIC3R_PCH=ON in INLONG_SLICER_CMAKE_ARGS still wins, because
    ...
    if not "%dry_run%" == "ON" if not "%no_configure%" == "ON" if not "%build_deps%" == "ON" if not exist "!DEP_TREE!\InlongSlicer_dep\usr\local\" (
    ...
    if not "%no_configure%" == "ON" (
        call :print_and_run cmake -B "%build_dir%" -G "%generator%" %gen_args% -DINLONG_TOOLS=ON %SIG_FLAG% %TESTS_FLAG% %DEP_TREE_FLAG% -DCMAKE_BUILD_TYPE=%build_type% !slicer_args! %INLONG_SLICER_CMAKE_ARGS%
        %error_check%
    )
...
:summary
    ...
    set "slicer_exe=%build_dir%\src\%build_type%\inlong-slicer.exe"
    if "%install_slicer%" == "ON" set "slicer_exe=%build_dir%\InlongSlicer\inlong-slicer.exe"
    for %%p in ("!slicer_exe!") do set "slicer_full=%%~fp"
    REM The 2026 generator writes InlongSlicer.slnx, the releases before it
    REM InlongSlicer.sln. A file already there wins, in case an older CMake
    REM configured the build.
    set "solution=InlongSlicer.sln"
    if "%vs_version%" == "2026" set "solution=InlongSlicer.slnx"
    if exist "!build_full!\InlongSlicer.sln" set "solution=InlongSlicer.sln"
    if exist "!build_full!\InlongSlicer.slnx" set "solution=InlongSlicer.slnx"
    ...
    if /I "%slicer_target%" == "InlongSlicer_app_gui" set "linked=ON"
    ...
    if "%build_slicer%" == "ON" if "%linked%" == "ON" echo   InlongSlicer    !slicer_full!
...
:print_help_msg
    ...
    echo Builds InlongSlicer and its dependencies on Windows.
    echo.
    ...
    echo.
    echo Environment:
    echo    INLONG_DEPS_CMAKE_ARGS      Extra arguments for the deps configure
    echo    INLONG_SLICER_CMAKE_ARGS    Extra arguments for the slicer configure
    echo    INLONG_UPDATER_SIG_KEY      Update signing key baked into the slicer
    ...
    echo.
    echo       set INLONG_SLICER_CMAKE_ARGS=-DSLIC3R_BUILD_SANDBOXES=ON -DSLIC3R_WARNINGS=OFF
    echo       $env:INLONG_SLICER_CMAKE_ARGS = '-DSLIC3R_BUILD_SANDBOXES=ON'      (PowerShell)
    echo.
