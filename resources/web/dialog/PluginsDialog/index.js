...
function RenderDescription(plugin) {
  ...
  // no header, so link to Inlong Cloud instead.
  ...
  if (isCloud && String(plugin?.sharing_token || "")) {
    node.appendChild(document.createTextNode("View on Inlong Cloud "));
...
function RenderDetailSummary(container, plugin) {
  ...
  if (plugin.orphaned === true) {
    ...
    warning.textContent = "Orphaned: This plugin is no longer subscribed or available in Inlong Cloud. The local copy remains installed and can still be used.";
