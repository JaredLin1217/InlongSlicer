...
- Build dirs on the dev machine: main app = `build_arm64/` (Xcode generator, multi-config), deps = `deps/build/arm64/` (Unix Makefiles). App target name: `InlongSlicer`. Substitute your own configured build dirs where noted.
...
```bash
cmake --build build_arm64 --config RelWithDebInfo --target InlongSlicer
```
...
```bash
otool -L build_arm64/src/RelWithDebInfo/InlongSlicer.app/Contents/MacOS/InlongSlicer | grep -i "libav" || echo "OK: no dynamic FFmpeg"
```
...
```bash
open build_arm64/src/RelWithDebInfo/InlongSlicer.app
```
