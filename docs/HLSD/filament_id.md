...
InlongFilamentLibrary (OFL), Qidi, or Snapmaker bundle. The granularity is the name on the spool,
not the brand behind it: `AAA PLA Lite` and `AAA PLA Pro` are two filaments with two ids, not
variants of one.

**How it is generated:** an id is computed, never invented. `scripts/inlong_id_tool.py`
...
InlongFilamentLibrary, InlongArena, Qidi, and Snapmaker bundles all arrive at independently
...
Klipper, Snapmaker), InlongSlicer matches the reported id against the filament presets
...
> `python scripts/inlong_id_tool.py --generate`; one already in the tree has one — inherit it.
...
- The machine-facing dialogs (AMS tray edit, AMS dry control, calibration history, extrusion
  ...
  puts Inlong Filament Library materials in those lists — deduplicated to one entry per
...
- The id is load-bearing at startup: an instantiated system filament (one marked
  ...
  InlongFilamentLibrary itself the failure is messier: library presets loaded before the
...
key needed). Tuning a generic material → **join the InlongFilamentLibrary filament** (inherit
...
1. **Every preset carries the id of its own product, wherever it gets it from.** The id is a
   ...
   InlongFilamentLibrary preset — and CI checks one thing: the id it ends up with equals the
...
4. **Generics belong to InlongFilamentLibrary.** A vendor tuning a generic material inherits
...
(the `setting_id` half of `scripts/inlong_id_tool.py`):
...
InlongFilamentLibrary never changes its id, and two vendors independently shipping the same
product arrive at the same id without coordinating. `Polymaker/PLA/PolyLite PLA` mints
`OF5CgdDq`, and that one id is declared by the InlongFilamentLibrary, InlongArena, Qidi, and
...
python scripts/inlong_id_tool.py --dry-run          # 2. preview the ids — writes nothing
python scripts/inlong_id_tool.py --generate         # 3. apply them to the profile file(s)
python scripts/inlong_id_tool.py --update-snapshot  # 4. record the new claims in the snapshot
python scripts/inlong_id_tool.py --check            # 5. validate the filament_id state
python scripts/inlong_extra_profile_check.py        # 6. ...and everything else CI checks
...
loudly), and a no-op on a tree that already passes `scripts/inlong_extra_profile_check.py` — the
...
the instruction to run `python scripts/inlong_id_tool.py --generate`; once the id is minted, the
...
Inlong BBL filaments with no row: 135 Inlong-only product(s)
```
...
InlongFilamentLibrary. **135 is the number to expect at every regeneration** — 109 was the
...
`to_inlong_filament_id` and `from_inlong_filament_id` returning their argument, and `BBLPrinterAgent`
...
`BBLPrinterAgent::to_inlong_payload` and `from_inlong_payload` rewrite every string under
...
Bambu-specific rules. `inlongslicer_discover_tests` registers a Catch2 tag as a CTest **label**,
...
- **A build configured without the GUI.** `target_link_libraries(InlongSlicer libslic3r_gui)` sits
  inside `if (SLIC3R_GUI)` in `src/CMakeLists.txt`, so the lookups are not linkable when the GUI
  is off. The CLI's 3mf writer in `src/InlongSlicer.cpp` therefore guards its translation with
...
`scripts/inlong_extra_profile_check.py`. Its ground truth is
...
`inlong_extra_profile_check.py` separately holds every declared id to the AMS 8-character limit,
...
- **A branded filament that borrows a generic's settings?** Fine — inherit `Generic X @System`
  ...
  `python scripts/inlong_id_tool.py --generate` to mint it. Inheritance never changes the id.
...
- **CI says my filament needs an id.** Run `python scripts/inlong_id_tool.py --generate`, then
