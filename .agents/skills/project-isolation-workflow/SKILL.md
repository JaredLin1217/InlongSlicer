...
description: Repo-local Agents rules, templates, memory, deploy.
---
# Project Isolation Workflow
Token-min router for isolation, memory, deploy, agents, handoff, skills, maintenance. Start `ai-runtime.yaml`; canonical YAML wins.
## Route
...
- Deploy: preserve layout; write only `deployed_file_set`; report historical files.
...
- Runtime cleanup/hiring/scoring: expand `workflows.yaml`; closeout runs `runtime.quiet_cleanup` for current parent/cwd closed subagent runtime ids, never sidebar nicknames; clear sqlite, session index, unread state, and rollouts; no backups.
