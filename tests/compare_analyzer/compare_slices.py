"""
Slicing G-code Comparison Tool: InlongSlicer vs BambuStudio (BBL)
====================================================================

The script performs a deep comparison of two `.3mf` slicing project files
(the first one from InlongSlicer, the second from Bambu Studio) for debugging the Vortek H2C nozzle changer.
...
def get_slicer_name(meta):
    ...
    if "InlongSlicer-Version" in version_info:
        return "InlongSlicer"
    elif "OrcaSlicer-Version" in version_info:
        return "OrcaSlicer"
    elif "X-BBL-Client-Version" in version_info:
        return "BambuStudio"
    for k in version_info.keys():
        if "inlongslicer" in k.lower():
            return "InlongSlicer"
...
def build_comparison_report(f1_name, f1_meta, f1_settings, f1_events, f1_lines, f1_size, f1_stats,
    ...
    v1_val = f1_meta["version_info"].get("InlongSlicer-Version") or f1_meta["version_info"].get("OrcaSlicer-Version") or f1_meta["version_info"].get("X-BBL-Client-Version") or "Unknown"
    v2_val = f2_meta["version_info"].get("InlongSlicer-Version") or f2_meta["version_info"].get("OrcaSlicer-Version") or f2_meta["version_info"].get("X-BBL-Client-Version") or "Unknown"
