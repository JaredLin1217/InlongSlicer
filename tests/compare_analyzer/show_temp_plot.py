...
Generates interactive HTML with temperature plots for analyzing and comparing InlongSlicer vs BambuStudio G-code.
...
2. Comparison Mode (Two Files):
   Renders a side-by-side alignment of temperature panels for both files (e.g. InlongSlicer vs BambuStudio).
   Usage:
...
def parse_file_data(filepath):
    ...
    try:
        ...
        slicer_name = "InlongSlicer" if "inlong" in filename_lower else (
            "OrcaSlicer" if "orca" in filename_lower else (
                "BambuStudio" if "bbl" in filename_lower else "Slicer"
            )
        )
