...
std::string inlong_id_of(const std::string& bambu_id)
{
    boost::nowide::ifstream file(resources_dir() + "/printers/bambu_filament_ids.json");
    json doc;
    file >> doc;
    for (const auto& [inlong_id, row] : doc["filaments"].items())
...
TEST_CASE("Bambu filament id map is one-to-one and leaves unmapped ids alone", "[BambuFilamentIds]")
{
    ...
    const std::string abs = inlong_id_of("GFB00");                      // Bambu ABS
    REQUIRE(abs.rfind("OF", 0) == 0);
    CHECK(bbl.from_inlong_filament_id(abs) == "GFB00");
    CHECK(bbl.to_inlong_filament_id("GFB00") == abs);
    CHECK(bbl.from_inlong_filament_id("OFnotarow") == "OFnotarow");
    CHECK(bbl.to_inlong_filament_id("GFZZ99") == "GFZZ99");             // Bambu id we do not ship
    CHECK(bbl.to_inlong_filament_id("P1234567") == "P1234567");         // user root
    CHECK(bbl.from_inlong_filament_id("") == "");

    // An agent whose printers already speak our ids inherits IPrinterAgent's identity default.
    const InlongPrinterAgent other{""};
    CHECK(other.from_inlong_filament_id(abs) == abs);
    CHECK(other.to_inlong_filament_id("GFB00") == "GFB00");
}

TEST_CASE("Payload rewrite covers nested trays, calibration lists and mapping info", "[BambuFilamentIds]")
{
    const ScopedRepoResourcesDir repo_resources;
    const std::string abs = inlong_id_of("GFB00"), pla = inlong_id_of("GFA00");
    ...
    json inbound = json::parse(BBLPrinterAgent::to_inlong_payload(status));
    ...
    CHECK(json::parse(BBLPrinterAgent::from_inlong_payload(inbound.dump())) == json::parse(status));    // round trip

    const std::string mapping = R"([{"ams":0,"filamentId":")" + abs + R"(","filamentType":"ABS"}])";
    CHECK(BBLPrinterAgent::from_inlong_payload(mapping) == R"([{"ams":0,"filamentId":"GFB00","filamentType":"ABS"}])");
    CHECK(BBLPrinterAgent::from_inlong_payload("not json") == "not json");
    CHECK(BBLPrinterAgent::from_inlong_payload(R"({"print":{"command":"pushall"}})") == R"({"print":{"command":"pushall"}})");
}
