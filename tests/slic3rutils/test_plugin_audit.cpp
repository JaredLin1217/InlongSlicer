...
#include <slic3r/Utils/InlongCloudServiceAgent.hpp> // secret_constants::USER_SECRET_FILENAME
...
TEST_CASE("Plugin audit denies app config and token filenames anywhere", "[audit]")
{
    ...
    {
        // The prefix is the full registered name ("InlongSlicer.conf"), not the stem "InlongSlicer",
        ...
        CHECK_FALSE(mgr.is_denied_filename(fs::path("inlong_refresh_token.txt")));
    }
...
TEST_CASE("Plugin audit does not constrain non-plugin code", "[audit]")
{
    ...
    PluginAuditManager& mgr = PluginAuditManager::instance();
    mgr.clear_current_plugin(); // no plugin context: this is InlongSlicer's own C++/internal Python
