...
{
    ScopedTemporaryDir dir;
    const fs::path      library_dir = dir.path() / PresetBundle::INLONG_FILAMENT_LIBRARY;
    const fs::path      vendor_dir  = dir.path() / "Acme";
    const fs::path      child_file  = vendor_dir / "filament" / "nested" / "petg.json";

    std::ofstream((dir.path() / (std::string(PresetBundle::INLONG_FILAMENT_LIBRARY) + ".json")).string())
        << R"({"version":"1.0.0","name":"InlongFilamentLibrary","filament_list":[)"
...
// INLONG: a filament in the Inlong Filament Library that names its compatible printers has to hide the generic
...
// an Inlong Filament Library generic with no compatible_printers, a same alias vendor filament
