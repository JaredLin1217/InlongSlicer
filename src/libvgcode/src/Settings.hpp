...
struct Settings
{
	  ...
	  // Visualization parameters
		...
		// INLONG: when enabled, every layer the layer slider is not scrubbed to is rendered
		// darkened (keeping its color) while showing less than the full print
		bool dim_previous_layers{ false };
		// INLONG: how bright those darkened layers are rendered, 1.0 = unchanged, 0.0 = black
