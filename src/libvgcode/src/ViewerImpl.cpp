  ...
  static constexpr const Vec3 ZERO = { 0.0f, 0.0f, 0.0f };
    ...
    for (size_t i = 0; i < vertices.size(); ++i) {
        ...
        if (heights_widths_angles != nullptr) {
            ...
            // INLONG: Set bias for wipes and options to avoid z-fighting
            float bias = 0.0f;
            if (v.is_wipe())
                bias = 0.05f;
            else if (v.is_option())
...
// INLONG: returns the encoded color scaled towards black by 'brightness', preserving its hue.
...
void ViewerImpl::update_colors_texture()
{
    ...
    // INLONG: when dim_previous_layers is enabled, darken every layer (keeping its color) except the
    ...
    for (size_t i=0; i<m_vertices.size(); ++i) {
        ...
        // INLONG: layers kept at full brightness by the dimming above are excluded from the greying below too
...
// INLONG: enable/disable darkening of the layers the layer slider is not scrubbed to
...
// INLONG: set how bright the darkened layers are rendered, 1.0 = unchanged, 0.0 = black
