...
#include <unistd.h>
    ...
    // BBLPrinterAgent::from_inlong_filament_id(); the map and its lookups live in libslic3r_gui,
    // which only a SLIC3R_GUI build links (see target_link_libraries(InlongSlicer libslic3r_gui)
    // in CMakeLists).
...
std::map<int, std::string> cli_errors = {
    ...
    {CLI_NO_SUITABLE_OBJECTS, "One of the plate is empty or has no object fully inside it. Please check that the 3mf contains no empty plate in Inlong Slicer before uploading."},
    {CLI_VALIDATE_ERROR, "There are some incorrect slicing parameters in the 3mf. Please verify the slicing of all plates in Inlong Slicer before uploading."},
    ...
    {CLI_FILAMENT_NOT_MATCH_BED_TYPE, "Filaments are not compatible with the plate type. Please verify the slicing of all plates in Inlong Slicer before uploading."},
    {CLI_FILAMENTS_DIFFERENT_TEMP, "The temperature difference of the filaments used is too large. Please verify the slicing of all plates in Inlong Slicer before uploading."},
    {CLI_OBJECT_COLLISION_IN_SEQ_PRINT, "Object conflicts were detected when using print-by-object mode. Please verify the slicing of all plates in Inlong Slicer before uploading."},
    {CLI_OBJECT_COLLISION_IN_LAYER_PRINT, "Object conflicts were detected. Please verify the slicing of all plates in Inlong Slicer before uploading."},
    {CLI_SPIRAL_MODE_INVALID_PARAMS, "Some slicing parameters cannot work with Spiral Vase mode. Please solve the issue in Inlong Slicer before uploading."},
    ...
    {CLI_SLICING_ERROR, "Failed slicing the model. Please verify the slicing of all plates on Inlong Slicer before uploading."},
    {CLI_GCODE_PATH_CONFLICTS, " G-code conflicts detected after slicing. Please make sure the 3mf file can be successfully sliced in the latest Inlong Slicer. If the file slices normally in Inlong Slicer, try moving the wipe tower further from other models, as we use more conservative parameters for it during upload."},
    {CLI_GCODE_PATH_IN_UNPRINTABLE_AREA, "Found G-code in unprintable area of multi-extruder printers after slicing. Please make sure the 3mf file can be successfully sliced in the latest Inlong Slicer."}
};
...
int CLI::run(int argc, char **argv)
{
    ...
    set_current_thread_name("inlongslicer_main");
    // Save the thread ID of the main thread.
    save_main_thread_id();

#ifdef __WXGTK__
    // Linux backend selection at runtime, based on GDK_BACKEND env var.
    {
    ...
    if (start_gui) {
        ...
        BOOST_LOG_TRIVIAL(info) << "begin to launch " << SLIC3R_APP_FULL_NAME << " GUI soon";
    ...
    BOOST_LOG_TRIVIAL(warning) << boost::format("cli mode, Current %1% Version %2%") % SLIC3R_APP_FULL_NAME % INLONGSLICER_VERSION;
        ...
        for (const std::string& file : m_input_files) {
            ...
            try {
                ...
                if (is_bbl_3mf)
                {
                    ...
                    Semver cli_ver = *Semver::parse(INLONGSLICER_VERSION);
                    if (!allow_newer_file && ((cli_ver.maj() < file_version.maj()) || ((cli_ver.maj() == file_version.maj()) && (cli_ver.min() < file_version.min())))){
                        BOOST_LOG_TRIVIAL(error) << boost::format("Version Check: File Version %1% not supported by current cli version %2%")%file_version.to_string() %INLONGSLICER_VERSION;
    ...
    for (auto const &opt_key : m_actions) {
        if (opt_key == "help") {
            ...
            m_print_config.save_to_json(m_config.opt_string(opt_key), std::string("project_settings"), std::string("project"), std::string(INLONGSLICER_VERSION));
...
#ifdef SLIC3R_GUI
                if (is_bbl_printer)
                    it->filament_id = bbl_agent.from_inlong_filament_id(it->filament_id);
#endif /* SLIC3R_GUI */
            }
                        ...
                        for (int i = 0; i < partplate_list.get_plate_count(); i++) {
                            ...
                            if (plate_data->plate_thumbnail.is_valid()) {
                                if ((plate_to_slice != 0) && (plate_to_slice != (i + 1))) {
                                    ...
                                    BOOST_LOG_TRIVIAL(info) << boost::format("plate %1% has a valid thumbnail, width %2%, height %3%嚗?directly using it")%(i+1) %plate_data->plate_thumbnail.width %plate_data->plate_thumbnail.height;
                                }
                            }
...
bool CLI::setup(int argc, char **argv)
{
...
#ifdef WIN32
    // Notify user that a blacklisted DLL was injected into InlongSlicer process (for example Nahimic, see GH #5573).
    // We hope that if a DLL is being injected into an InlongSlicer process, it happens at the very start of the application,
    // thus we shall detect them now.
    if (BlacklistedLibraryCheck::get_instance().perform_check()) {
        std::wstring text = L"Following DLLs have been injected into the Inlong Slicer process:\n\n";
        text += BlacklistedLibraryCheck::get_instance().get_blacklisted_string();
        text += L"\n\n"
                L"Inlong Slicer is known to not run correctly with these DLLs injected. "
                L"We suggest stopping or uninstalling these services if you experience "
                L"crashes or unexpected behaviour while using Inlong Slicer.\n"
                L"For example, ASUS Sonic Studio injects a Nahimic driver, which makes Inlong Slicer "
    ...
    // Path from Slic3r binary to resources:
    boost::filesystem::path path_resources = boost::filesystem::canonical(path_to_binary).parent_path().parent_path() / "resources";
    //Inlong: for build systems that support multiple configurations, the binary may be in a subdirectory like "bin/Release" or "bin/Debug".
...
void CLI::print_help(bool include_print_options, PrinterTechnology printer_technology) const
{
    ...
    boost::nowide::cout
        ...
        << std::endl
        << "Usage: " << SLIC3R_APP_CMD << " [ OPTIONS ] [ file.3mf/file.stl ... ]" << std::endl
        << std::endl
        << "OPTIONS:" << std::endl;
...
extern "C" {
    __declspec(dllexport) int __stdcall inlongslicer_main(int argc, wchar_t **argv)
    {
...
int main(int argc, char **argv)
{
    ...
    // InlongSlicer with SIGPIPE (exit 141) and produces no crash report.
