...
{
    // Match the application's UTF-8 path contract before parsing arguments or
    // enumerating presets. The system code page can lose names such as Desactive
    // with accented characters, even when the profile root itself is ASCII.
    boost::nowide::args utf8_args(argc, argv);
    boost::nowide::nowide_filesystem();

    po::options_description desc("InlongSlicer Profile Validator\nUsage");
    // clang-format off
