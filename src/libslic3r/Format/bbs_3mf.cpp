...
#define _(s) Slic3r::I18N::translate(s)
        ...
        int plate_id)
    {
        ...
        if (!m_is_bbl_3mf) {
            // if the 3mf was not produced by InlongSlicer and there is more than one instance,
...
//        model.adjust_min_z();
    ...
    bool _BBS_3MF_Importer::_handle_end_model()
    {
        ...
        if (!m_is_bbl_3mf) {
            // if the 3mf was not produced by InlongSlicer and there is only one object,
    ...
    bool _BBS_3MF_Importer::_generate_volumes(ModelObject& object, const Geometry& geometry, const ObjectMetadata::VolumeMetadataList& volumes, ConfigSubstitutionContext& config_substitutions)
    {
        ...
        for (const ObjectMetadata::VolumeMetadata& volume_data : volumes) {
            ...
            if (!m_is_bbl_3mf) {
                // if the 3mf was not produced by InlongSlicer and there is only one instance,
                // bake the transformation into the geometry to allow the reload from disk command
                // to work properly
                if (object.instances.size() == 1) {
                    triangle_mesh.transform(object.instances.front()->get_transformation().get_matrix(), false);
