...
// Inlong: input shaping values types by flavor
...
void PrintConfigDef::init_common_params()
{
    ...
    def->tooltip = L("Enable this if the printer accepts a 3MF file as the print job. When enabled, Inlong Slicer "
    ...
    def->tooltip = L("Inlong Slicer can upload G-code files to a printer host. This field should contain "
...
void PrintConfigDef::init_fff_params()
{
    ...
    // Inlong: make sure the order of the values is the same as the BedType enum
    ...
    // Inlong: allow profile maker to set default bed type in machine profile
    ...
    def->sidetext = L("layers"); // INLONG add side text
    ...
    def->tooltip = L(
        ...
        "set appropriately. Inlong Slicer ensures that adaptive_bed_mesh_min/adaptive_bed_mesh_max values do not exceed these min/max "
    ...
    // Inlong: Adaptive pressure advance option and calibration values
    ...
    */
    ...
    def->tooltip = L("After a tool change, the exact position of the newly loaded filament inside "
                     ...
                     "Before purging the print head into an infill or a sacrificial object, Inlong Slicer will always prime "
    ...
    def->sidetext = L("layer"); // INLONG add side text
    def->min = 0;
    def->max = 1000;
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionInts { 0 });
    ...
    // INLONG: minimum non-zero part cooling fan speed.
    ...
    // Inlong: may remove this option later
    ...
    def->tooltip = L("Do not write the CONFIG_BLOCK (slicer configuration key/value pairs) into the G-code file. "
                   ...
                   "so importing it back into InlongSlicer will not restore the configuration.");
    def->mode = comAdvanced;
    def->set_default_value(new ConfigOptionBool(false));
    ...
    def->tooltip = L("Connect an infill line to an internal perimeter with a short segment of an additional perimeter. "
                     ...
                     "Inlong Slicer tries to connect two close infill lines to a short perimeter segment. If no such perimeter segment "
    ...
    // INLONG: special flag for flow rate calibration
    ...
    def->tooltip = L("Inlong Slicer can upload G-code files to a printer host. This field must contain "
    ...
    def->tooltip = L("If you want to process the output G-code through custom scripts, "
                   ...
                   "and they can access the Inlong Slicer config settings by reading environment variables.");
    def->gui_flags = "serialized";
    def->multiline = true;
    def->full_width = true;
    def->height = 6;
    ...
    def->tooltip = L("To reduce the waiting time after tool change, Inlong Slicer can preheat the next tool while the current tool is still in use. "
                     "This setting specifies the time in seconds to preheat the next tool. Inlong Slicer will insert a M104 command to preheat the tool in advance.");
    def->sidetext = L("s");	// seconds, CIS languages need translation
    def->min = 0;
    def->max = 120;
    def->mode = comAdvanced;
    ...
    def->tooltip = L("Force the toolhead to travel to the wipe tower before issuing the tool change command (Tx). "
                     ...
                     "By default Inlong skips the travel on multi-toolhead machines because the firmware handles the head swap, "
...
void PrintConfigDef::handle_legacy(t_config_option_key &opt_key, std::string &value)
{
    ...
    // Inlong: Rename wipe tower ribs related options
...
// Inlong: Special handling for extruder variants
...
CLIMiscConfigDef::CLIMiscConfigDef()
{
    ...
    def->tooltip = L("This version of Inlong Slicer may not understand configurations produced by the newest Inlong Slicer versions. "
                     "For example, newer Inlong Slicer may extend the list of supported firmware flavors. One may decide to "
    ...
    def->tooltip = L("If enabled, the command line arguments are sent to an existing instance of GUI Inlong Slicer, "
                     "or an existing Inlong Slicer window is activated. "
...
*/
    ...
    def->label = L("Allow filaments with high/low temperature to be printed together");
    def->tooltip = L("Allow filaments with high/low temperature to be printed together.");
    def->cli_params = "option";
    def->set_default_value(new  ConfigOptionBool(false));
}
...
ReadWriteSlicingStatesConfigDef::ReadWriteSlicingStatesConfigDef()
{
    ...
    def->tooltip = L("Position of the extruder at the beginning of the custom G-code block. If the custom G-code travels somewhere else, "
                     "it should write to this variable so Inlong Slicer knows where it travels from when it gets control back.");
    ...
    def->tooltip = L("Retraction state at the beginning of the custom G-code block. If the custom G-code moves the extruder axis, "
                     "it should write to this variable so Inlong Slicer de-retracts correctly when it gets control back.");
...
OtherSlicingStatesConfigDef::OtherSlicingStatesConfigDef()
{
    ...
    // Options from PS not used in Inlong
