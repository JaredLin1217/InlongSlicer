                                                        ...
                                                        AppConfig&                            config)
{
    ...
    for (auto &file : files) {
        ...
        if (boost::iends_with(file, ".inlong_printer") || boost::iends_with(file, ".inlong_bundle") ||
            boost::iends_with(file, ".inlong_filament") || boost::iends_with(file, ".orca_printer") ||
            boost::iends_with(file, ".orca_bundle") || boost::iends_with(file, ".orca_filament") ||
    ...
    const std::set<std::string>* retain_configs)
{
    ...
    if (flags.has(LoadConfigBundleAttribute::LoadSystem)) {
        ...
        // matching scripts/inlong_id_tool.py. Only instantiated presets carry an id;
    ...
    ForwardCompatibilitySubstitutionRule compatibility_rule, const PresetBundle* base_bundle, bool allow_cache)
{
    ...
    auto parse_subfile = [this, dir, vendor_name](
        ...
        try {
            ...
            if (key_values.find(INLONG_JSON_KEY_RENAMED_FROM) != key_values.end()) {
                if (!unescape_strings_cstyle(key_values[INLONG_JSON_KEY_RENAMED_FROM], entry.renamed_from)) {
    ...
    // The section order below — process, filaments (with the Inlong library map copy),
    ...
    load_subfiles(filament_subfiles, cache_data.filament_entries, "filament", is_inlong_lib);
    if (is_inlong_lib) {
...
// Inlong: turn a preset file path into an absolute file:// URI for log messages.
...
// Inlong: validator-only. Flag any system preset whose inherits / compatible_printers /
...
// / compatible_prints only name same-vendor or InlongFilamentLibrary presets (both loaded), so a
...
// Inlong: a filament is matched from the AMS by (filament_id + printer compatibility).
...
bool PresetBundle::check_duplicate_filament_subtypes() const
{
    ...
    const auto library_it = filaments_by_vendor.find(INLONG_FILAMENT_LIBRARY);
                                     ...
                                     const Semver& expected_vendor_version, const PresetBundle* base_bundle)
{
    ...
    try {
        ...
        const bool is_inlong_lib = vendor_name == INLONG_FILAMENT_LIBRARY;
        install_entries(data.filament_entries, &this->filaments, is_inlong_lib);
        if (is_inlong_lib) {
            m_config_maps      = configs;
            m_filament_id_maps = filament_id_maps;
        }
