...
{
    this->set("version", std::string(INLONGSLICER_VERSION));
...
namespace client
{
    ...
    struct MyContext : public ConfigOptionResolver {
           ...
           OptWithPos       &output)
        {
            if (! ctx->skipping()) {
                if (index < 0)
                    index = 0; // Inlong: fallback to first element if index < 0, this matches the behavior of BambuStudio
                if (!opt.opt->is_vector())
                    index = -1; // Inlong: ignore the index if variable is scalar, this matches the behavior of BambuStudio
                output = opt;
                output.index = index;
            } else
                output = opt;
