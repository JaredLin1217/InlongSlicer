...
//INLONG: Brim can follow the post-EFC outline when enabled.
...
//INLONG: Helper for snapping painted ears to the EFC outline.
...
//INLONG: Helper for matching painted ears to their original island before EFC snapping.
...
//INLONG: Keep painted ear snapping on the matching island when using EFC outline.
...
//INLONG: Use post-processed first-layer slices (including EFC) for brim outline.
...
static ExPolygons make_brim_ears(const PrintObject* object, const double& flowWidth, float brim_offset, Flow &flow, bool is_outer_brim)
{
    ...
    //INLONG: Painted ears can snap to the EFC-adjusted outline when enabled.
    const bool use_efc_outline = use_brim_efc_outline(*object);
    const ExPolygons &raw_outline = object->layers().front()->lslices;
    //INLONG: Lazily computed EFC-adjusted bottom outline.
    ...
    for (auto &pt : brim_ear_points) {
        ...
        //INLONG: Snap painted ears to the EFC-adjusted outline when enabled.
        if (use_efc_outline) {
            if (efc_outline == nullptr) {
                //INLONG: Compute EFC-adjusted outline lazily for painted ear snapping.
            ...
            if (!efc_outline->empty()) {
                Point closest_point;
                //INLONG: Snap within the matching island to avoid drifting to another island.
    ...
    std::vector<unsigned int>& printExtruders)
{
    ...
    for (unsigned int extruderNo : printExtruders) {
        ...
        for (const auto& objectWithExtruder : objPrintVec) {
            ...
            //INLONG: Select brim base slices from EFC-compensated outline when enabled.
            const bool         use_efc_outline = use_brim_efc_outline(*object);
            ExPolygons         brim_slices_storage;
            const ExPolygons*  brim_slices = nullptr;
            //INLONG: Select EFC-adjusted bottom outline when enabled.
            if (use_efc_outline)
            ...
            if (objectWithExtruder.second == extruderNo && brimToWrite.at(object->id()).obj) {
                ...
                for (const auto& volumeGroup : object->firstLayerObjGroups()) {
                    ...
                    if (use_efc_outline) {
                        //INLONG: When using EFC outline, restrict per-volume-group slices to the
    ...
    // Inlong: if per-extruder print area is not specified, use the whole bed as printable area for all extruders
    ...
    std::map<ObjectID, ExPolygons>* supportBrimAreasOut)
{
    ...
    for (const ObjectID printObjID : print.print_object_ids()) {
        ...
        //INLONG: Use EFC-compensated outline for brim bounding box when enabled.
    ...
    // Inlong: keep translated brim footprints for skirt grouping.
    ...
    if (!combine_brims) {
        // Inlong: Generate brims separately when brims cannot be combined.
