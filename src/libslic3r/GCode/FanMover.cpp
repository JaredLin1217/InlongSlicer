...
const std::string& FanMover::process_gcode(const std::string& gcode, bool flush)
{
    ...
    if (flush) {
        while (!m_buffer.empty()) {
            ...
            // Inlong: Keep the emitted fan state in sync when flushing buffered fan commands.
...
std::string FanMover::_set_fan(int16_t speed) {
    ...
    // INLONG: apply the per-printer non-zero fan PWM floor so reposted fan commands respect the clamp too.
