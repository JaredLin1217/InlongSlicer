    ...
    for (size_t i = 1; i < input_points.size(); i++) {
        ...
        if (ADD_INTERSECTIONS &&
            ...
            for (const auto &intersection : intersections) {
                ...
                // INLONG: Filter out points that are introduced at intersections if their distance from the previous or next point is not meaningful
    ...
    if (PREV_LAYER_BOUNDARY_OFFSET && ADD_INTERSECTIONS) {
        ...
        for (int point_idx = 0; point_idx < int(points.size()) - 1; ++point_idx) {
            ...
            if ((curr.distance > -boundary_offset && curr.distance < boundary_offset + 2.0f) ||
                ...
                // INLONG: Segment path to smaller lines by adding additional points only if the path has an overhang that
                ...
                if ((min_distance > 0 && ((std::abs(curr.distance) > min_distance) || (std::abs(next.distance) > min_distance)) && line_len >= 2.f) ||
                    ...
                    if (t0 < 1.0) {
                        ...
                        // INLONG: only create a new point in the path if the new point overhang distance will be used to generate a speed change
                        // or if this option is disabled (min_distance<=0)
                        if( (std::abs(p0_dist) > min_distance) || (min_distance<=0)){
                            // INLONG: also filter out points that are introduced to the start of the path when their distance from the start point is
                            // not meaningful
                    ...
                    if (t1 > 0.0) {
                        ...
                        if( (std::abs(p1_dist) > min_distance) || (min_distance<=0)){
                            // INLONG: filter out points that are introduced to the end of the path when their distance from the end point is
                            // not meaningful
        ...
        new_points.reserve(points.size() * 2);
        {
            for (size_t i = 0; i + 1 < points.size(); i++) {
                ...
                for (size_t j = 1; j < new_point_count + 1; j++) {
                    ...
                    // INLONG: Filter out points that are introduced if their distance from the previous or next point is not meaningful
...
public:
                                                           ...
                                                           bool								   slowdown_for_curled_edges)
    {
        ...
        // Inlong: Find the smallest overhang distance where speed adjustments begin
        ...
        // Inlong: Pass to the point properties estimator the smallest ovehang distance that triggers a slowdown (smallest_distance_with_lower_speed)
        ...
        for (size_t i = 0; i < extended_points.size(); i++) {
            ...
            // INLONG: Clamp resulting speed to lowest of calculated speed based on the overhang values and the current speed
