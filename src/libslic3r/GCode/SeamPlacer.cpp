...
void compute_global_occlusion(GlobalModelInfo &result, const PrintObject *po,
  ...
  for (const ModelVolume *model_volume : po->model_object()->volumes) {
    if (model_volume->type() == ModelVolumeType::MODEL_PART
      ...
      // INLONG: Mirrored transforms flip winding, keep normals outward
  ...
  // INLONG: Mirroring flips normals, keep them outward for visibility sampling
...
void gather_enforcers_blockers(GlobalModelInfo &result, const PrintObject *po) {
  ...
  for (const ModelVolume *mv : po->model_object()->volumes) {
    if (mv->is_seam_painted()) {
      ...
      // INLONG: Keep normals outward when mirroring seam enforcers
      ...
      // INLONG: Keep normals outward when mirroring seam blockers
