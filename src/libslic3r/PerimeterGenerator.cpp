    ...
    bool &steep_overhang_contour, bool &steep_overhang_hole, bool reverse_thin_wall_hole)
{
    ...
    for (const PerimeterGeneratorLoop &loop : loops) {
        ...
        if (perimeter_generator.config->detect_overhang_wall && perimeter_generator.layer_id > perimeter_generator.object_config->raft_layers) {
            ...
            // Always reverse extrusion if use fuzzy skin. Ref: upstream pull request #2413.
    ...
    for (const std::pair<size_t, bool> &idx : chain) {
        ...
        if (idx.first >= loops.size()) {
            ...
            // Inlong: Reverse print order for thin wall holes.
    ...
    for (PerimeterGeneratorArachneExtrusion& pg_extrusion : pg_extrusions) {
        ...
        if (!paths.empty()) {
            if (extrusion->is_closed) {
                ...
                // Inlong: Reverse the order of paths for thin wall holes. We define thin wall hole as a hole with only one perimeter.
...
// INLONG: only_one_wall_top acts on top surfaces, so without a top shell there is nothing for it to act on: zero top
...
// INLONG: only_one_wall_first_layer thins the first layer to a single wall, the bottom counterpart of the above and
...
// INLONG: the inner walls are only given up when a top fill takes their space, and it has to actually reach it -
...
// INLONG: only_one_wall_top - cheap per-vertex classification of a wall against the top surface. Only Partial
...
// INLONG: only_one_wall_top for Arachne - cut out of the already generated inner walls the parts running over the top
...
void PerimeterGenerator::process_classic()
{
    ...
    // Inlong: ignore precise_outer_wall if wall_sequence is not InnerOuter
    ...
    // INLONG: neither one-wall option has a surface to act on without the shell behind it, see
    ...
    for (size_t order_idx = 0; order_idx < surface_order.size(); order_idx++) {
        ...
        // INLONG: only_one_wall_top, all empty unless this island has a top surface on this layer. See the
        ...
        if (loop_number >= 0) {
            ...
            for (int i = 0;; ++ i) {  // outer loop is 0
                ...
                if (i == 0 && i!=loop_number && only_one_wall_top && !surface.is_bridge() && this->upper_slices != NULL) {
                    if (top_fill_replaces_inner_walls(*this->config)) {
                        // INLONG: take the top fill and the keep-out region but leave `last` as the real geometry,
            ...
            if (is_outer_wall_first ||
                ...
                entities.reverse();
            // Inlong: sandwich mode. Apply after 1st layer.
        ...
        if (! gaps.empty()) { // collapse
            // INLONG: Use the smaller width as the lower bound to avoid overestimating safe overlap
...
#endif
            // Inlong: filter out tiny gap fills
        ...
        // INLONG: only_one_wall_top - route the top fill around the walls kept despite grazing the top.
...
// Inlong: sacrificial bridge layer algorithm ported from SuperSlicer
...
void PerimeterGenerator::process_arachne()
{
    ...
    for (const Surface& surface : all_surfaces) {
        ...
        // Inlong: set the topmost layer to be one wall according to the config
        ...
        if (inner_loop_number >= 0) {
            ...
            if (!top_expolygons.empty()) {
                ...
                // INLONG: skip if the top surface area is smaller than "min_width_top_surface"
                ...
                // INLONG: Expand the polygon with half the perimeter width in addition to the contracted amount,
                ...
                // INLONG: onion the real region (inside the outer wall) so the remaining walls follow the actual
        ...
        if (this->config->wall_sequence == WallSequence::InnerOuterInner && layer_id > 0) { // only enable inner outer inner algorithm after first layer
            if (ordered_extrusions.size() > 2) { // 3 walls minimum needed to do inner outer inner ordering
                ...
                coord_t threshold_external = (apply_precise_outer_wall)
                    // Precise outer wall ??use ?ull external spacing??
