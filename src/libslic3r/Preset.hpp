...
// scripts/inlong_id_tool.py and recomputed here when a profile ships without it.
// MUST stay byte-identical to scripts/inlong_id_tool.py.
// This is NOT the per-user cloud-sync setting_id
// (InlongCloudServiceAgent::generate_uuid_for_setting_id) - do not conflate them.
    ...
    std::deque<Preset>::iterator find_preset_internal(const std::string &name, bool from_inlong_lib_only = false)
    {
        ...
        if (it == m_presets.end() || it->name != name) {
            ...
            for (size_t i = 0; i < m_num_default_presets; ++ i)
                if (m_presets[i].name == name && (!from_inlong_lib_only || m_presets[i].m_from_inlong_filament_lib)) {
