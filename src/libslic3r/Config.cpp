...
ConfigSubstitutions ConfigBase::load_from_gcode_file(const std::string &file, ForwardCompatibilitySubstitutionRule compatibility_rule)
{
    ...
    {
        //BBS
        std::string app_gcode_header = std::string("; generated by ") + SLIC3R_APP_NAME;
        ...
        while (std::getline(ifs, header)) {
            ...
            // BBS
            if (strncmp(app_gcode_header.c_str(), line_c, strlen(app_gcode_header.c_str())) == 0) {
                header_found = true;
                break;
            }
        }
