...
std::string PythonInterpreter::bundled_uv_path()
{
    ...
    const fs::path configured_uv = INLONG_BUNDLED_UV_EXECUTABLE;
...
bool PythonInterpreter::initialize()
{
    ...
    try {
        ...
        if (python_home.empty()) {
            ...
            BOOST_LOG_TRIVIAL(error) << "Configured bundled Python root: " << INLONG_BUNDLED_PYTHON_ROOT;
#ifdef _WIN32
...
// Verify Python standard library directory exists
        ...
        // Set program name
        status = PyConfig_SetBytesString(&config, &config.program_name, "InlongSlicer");
        if (PyStatus_Exception(status)) {
