...
void CloudPluginService::set_cloud_agent(std::shared_ptr<InlongCloudServiceAgent> agent) { m_orca_agent = std::move(agent); }

std::shared_ptr<InlongCloudServiceAgent> CloudPluginService::get_cloud_agent() const { return m_orca_agent; }

bool CloudPluginService::can_fetch_cloud_plugins() const
{
    if (!m_orca_agent) {
        BOOST_LOG_TRIVIAL(warning) << "Inlong Cloud service agent is null";
        return false;
    }
