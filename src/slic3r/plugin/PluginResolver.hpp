...
struct MissingPlugin
{
    ...
    std::string recovery_url; // Inlong Cloud search URL (local plugins only)
    std::string opt;
...
// Opens the Inlong Cloud plugin hub (a search when there is exactly one ref, the hub otherwise).
