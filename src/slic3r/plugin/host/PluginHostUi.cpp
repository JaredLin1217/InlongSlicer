...
auto run_on_ui_blocking(Fn&& fn) -> std::invoke_result_t<Fn&>
{
    ...
    if (wxTheApp == nullptr)
        throw std::runtime_error("InlongSlicer application is not initialized");
...
void PluginHostUi::RegisterBindings(pybind11::module_& host)
{
    ...
    ui.def("message", &ui_message, py::arg("text"), py::arg("title") = "InlongSlicer", py::arg("buttons") = "ok",
           py::arg("icon") = "info",
    ...
    ui.def("create_window", &ui_create_window, py::arg("html"), py::arg("title") = "InlongSlicer", py::arg("width") = 820,
