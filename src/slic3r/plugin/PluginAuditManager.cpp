...
bool PluginAuditManager::is_denied_filename(const boost::filesystem::path& candidate) const
{
    ...
    // because filename() of data_dir()/plugins/../InlongSlicer.conf is already "InlongSlicer.conf",
    // and the prefix rule covers the .bak/.tmp companions that hold the same secrets plus Windows
    // alternate data streams ("InlongSlicer.conf:stream").  A plugin that launders a denied file
...
void PluginAuditManager::install_hook()
{
    ...
    // InlongCloudServiceAgent::set_config_dir runs during networking init — neither strictly
    // precedes the other, and if Inlong Cloud never initializes an owner-registered token deny
