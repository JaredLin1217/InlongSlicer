...
struct BambuFilamentIdMap { std::unordered_map<std::string, std::string> to_bambu, to_inlong; };
...
const BambuFilamentIdMap& bambu_filament_id_map()
{
    static const BambuFilamentIdMap map = [] {
        ...
        try {
            ...
            json doc;
            file >> doc;
            for (const auto& [inlong_filament_id, row] : doc.at("filaments").items()) {
                const std::string bambu_id = row.at("bambu_id").get<std::string>();
                m.to_bambu.emplace(inlong_filament_id, bambu_id);
                m.to_inlong.emplace(bambu_id, inlong_filament_id);
            }
...
OnMessageFn to_inlong_messages(OnMessageFn fn)
{
    if (!fn)
        return fn;
    return [fn = std::move(fn)](std::string dev_id, std::string msg) { fn(std::move(dev_id), BBLPrinterAgent::to_inlong_payload(std::move(msg))); };
}
...
std::string BBLPrinterAgent::to_inlong_filament_id(const std::string& printer_filament_id) const
{
    const auto& map = bambu_filament_id_map().to_inlong;
    auto it = map.find(printer_filament_id);
    return it != map.end() ? it->second : printer_filament_id;
}

std::string BBLPrinterAgent::from_inlong_filament_id(const std::string& inlong_filament_id) const
{
    const auto& map = bambu_filament_id_map().to_bambu;
    auto it = map.find(inlong_filament_id);
    return it != map.end() ? it->second : inlong_filament_id;
}

std::string BBLPrinterAgent::to_inlong_payload(std::string json_text)
{
    return rewrite_filament_ids(std::move(json_text), bambu_filament_id_map().to_inlong);
}

std::string BBLPrinterAgent::from_inlong_payload(std::string json_text)
{
...
{
    json_str = from_inlong_payload(std::move(json_str));
...
int dispatch_start(CurrentFn func, PrintParams& params, const CallbackFns&... callbacks)
{
    ...
    if (!func || !agent)
        return -1;
    params.ams_mapping_info = BBLPrinterAgent::from_inlong_payload(std::move(params.ams_mapping_info));
    switch (plugin.network_abi()) {
    case NetworkAbi::Legacy:
...
int BBLPrinterAgent::set_on_message_fn(OnMessageFn fn)
{
    ...
    if (func && agent) {
        return func(agent, to_inlong_messages(std::move(fn)));
    }
    return -1;
}
