...
{
    const auto& key = (provider.empty() || provider == INLONG_CLOUD_PROVIDER) ? INLONG_CLOUD_PROVIDER : provider;
...
std::string NetworkAgent::to_inlong_filament_id(const std::string& printer_filament_id) const
{
    if (m_printer_agent)
        return m_printer_agent->to_inlong_filament_id(printer_filament_id);
    return printer_filament_id;
}

std::string NetworkAgent::from_inlong_filament_id(const std::string& inlong_filament_id) const
{
    if (m_printer_agent)
        return m_printer_agent->from_inlong_filament_id(inlong_filament_id);
    return inlong_filament_id;
}
