...
void InlongCloudServiceAgent::persist_refresh_token(const std::string& token)
{
    if (token.empty()) {
        clear_refresh_token();
        return;
...
bool InlongCloudServiceAgent::load_refresh_token(std::string& out_token)
{
    out_token.clear();
...
void InlongCloudServiceAgent::clear_refresh_token()
{
    wxSecretStore store = wxSecretStore::GetDefault();
    if (store.IsOk()) {
