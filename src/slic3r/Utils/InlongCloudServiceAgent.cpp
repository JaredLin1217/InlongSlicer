...
constexpr const char* INLONG_DEFAULT_PUB_KEY = "sb_publishable_lvVe_whOi80SU9BPSxM1kA_tbt9AbR_";
...
constexpr const char* INLONG_SYNC_STATE_FILE    = "sync_state";
constexpr const char* INLONG_SYNC_PROFILE_TABLE = "profiles";
constexpr size_t INLONG_SYNC_MAX_PAYLOAD_SIZE   = 1048576; // 1MB size limit

constexpr const char* INLONG_SUBSCRIBED_PLUGINS  = "/api/v1/plugins/subscriptions";
constexpr const char* INLONG_UNSUBSCRIBE_PLUGINS = "/api/v1/plugins/subscriptions";
...
constexpr const char* CONFIG_INLONG_API_URL   = "inlong_api_url";
constexpr const char* CONFIG_INLONG_AUTH_URL  = "inlong_auth_url";
constexpr const char* CONFIG_INLONG_CLOUD_URL = "inlong_cloud_url";
constexpr const char* CONFIG_INLONG_PUB_KEY   = "inlong_pub_key";

constexpr const char* SECRET_STORE_SERVICE = "InlongSlicer/Auth";
constexpr const char* SECRET_STORE_USER    = "inlong_refresh_token";
constexpr std::chrono::seconds TOKEN_REFRESH_SKEW{900}; // 15 minutes

// Cross-process advisory lock serializing refresh-token rotation between InlongSlicer instances on
...
#if defined(_WIN32)
        const std::wstring name = L"Local\\InlongTokenRefresh_" + std::to_wstring(std::hash<std::string>{}(path));
...
int choose_loopback_port()
{
    ...
    if (const char* env_port = std::getenv("INLONG_LOOPBACK_PORT")) {
        try {
            ...
            BOOST_LOG_TRIVIAL(warning) << "InlongCloudServiceAgent: invalid INLONG_LOOPBACK_PORT value, falling back to default";
        }
    }
...
InlongCloudServiceAgent::InlongCloudServiceAgent(std::string log_dir)
    : log_dir(std::move(log_dir))
    , api_base_url(INLONG_DEFAULT_API_URL)
    , auth_base_url(INLONG_DEFAULT_AUTH_URL)
    , cloud_base_url(INLONG_DEFAULT_CLOUD_URL)
{
    auth_headers["apikey"]    = INLONG_DEFAULT_PUB_KEY;
...
InlongCloudServiceAgent::~InlongCloudServiceAgent()
{
...
std::string InlongCloudServiceAgent::generate_uuid_for_setting_id(const std::string& name, const std::string& user_id)
{
    if (name.empty()) {
        return "";
    }
...
void InlongCloudServiceAgent::configure_urls(AppConfig* app_config)
{
    if (!app_config)
        return;
    ...
    std::string api_url = app_config->get(CONFIG_INLONG_API_URL);
    if (!api_url.empty()) {
        api_base_url = api_url;
    }

    std::string auth_url = app_config->get(CONFIG_INLONG_AUTH_URL);
    if (!auth_url.empty()) {
        auth_base_url = auth_url;
    }

    std::string cloud_url = app_config->get(CONFIG_INLONG_CLOUD_URL);
    if (!cloud_url.empty()) {
        cloud_base_url = cloud_url;
    }

    std::string pub_key = app_config->get(CONFIG_INLONG_PUB_KEY);
    if (!pub_key.empty()) {
        auth_headers["apikey"] = pub_key;
    }
}

void InlongCloudServiceAgent::set_api_base_url(const std::string& url) { api_base_url = url; }

void InlongCloudServiceAgent::set_auth_base_url(const std::string& url) { auth_base_url = url; }

void InlongCloudServiceAgent::set_cloud_base_url(const std::string& url) { cloud_base_url = url; }

void InlongCloudServiceAgent::set_use_encrypted_token_file(bool use) { m_use_encrypted_token_file = use; }

bool InlongCloudServiceAgent::get_use_encrypted_token_file() const { return m_use_encrypted_token_file; }

// ============================================================================
// ICloudServiceAgent - Lifecycle Methods
// ============================================================================

int InlongCloudServiceAgent::init_log() { return BAMBU_NETWORK_SUCCESS; }

int InlongCloudServiceAgent::set_config_dir(std::string cfg_dir)
{
    config_dir = cfg_dir;
...
int InlongCloudServiceAgent::set_cert_file(std::string folder, std::string filename)
{
    // Not used by InlongCloudServiceAgent (OAuth doesn't need client certs)
    (void) folder;
    (void) filename;
    return BAMBU_NETWORK_SUCCESS;
}

int InlongCloudServiceAgent::set_country_code(std::string code)
{
    country_code = code;
    return BAMBU_NETWORK_SUCCESS;
}
...
static bool parse_stored_secret(const std::string& secret, std::string& out_refresh_token, InlongCloudServiceAgent::SessionInfo& out_session)
{
    out_refresh_token.clear();
    out_session = InlongCloudServiceAgent::SessionInfo{};
    ...
    try {
        ...
        InlongCloudServiceAgent::SessionInfo user_session{};
        ...
        if (user_session.refresh_token.empty() || user_session.user_id.empty()) {
            BOOST_LOG_TRIVIAL(warning) << "InlongCloudServiceAgent: secret does not contain valid user session, force re-authentication";
            return false;
        }
        ...
        BOOST_LOG_TRIVIAL(error) << "InlongCloudServiceAgent: parse_stored_secret exception, force re-authentication";
        return false;
    }
}

int InlongCloudServiceAgent::start()
{
    regenerate_pkce();
...
bool InlongCloudServiceAgent::exchange_auth_code(const std::string& auth_code, const std::string& state, std::string& session_payload)
{
...
int InlongCloudServiceAgent::change_user(std::string user_info)
{
    try {
...
bool InlongCloudServiceAgent::is_user_login()
{
    std::lock_guard<std::mutex> lock(session_mutex);
    return session.logged_in;
}

int InlongCloudServiceAgent::user_logout(bool request)
{
...
std::string InlongCloudServiceAgent::get_user_id()
{
    std::lock_guard<std::mutex> lock(session_mutex);
    return session.user_id;
}

std::string InlongCloudServiceAgent::get_user_name()
{
    std::lock_guard<std::mutex> lock(session_mutex);
    return session.user_name;
}

std::string InlongCloudServiceAgent::get_user_avatar()
{
    std::lock_guard<std::mutex> lock(session_mutex);
    return session.user_avatar;
}

std::string InlongCloudServiceAgent::get_user_nickname()
{
    std::lock_guard<std::mutex> lock(session_mutex);
...
std::string InlongCloudServiceAgent::build_login_cmd()
{
...
std::string InlongCloudServiceAgent::build_logout_cmd() { return json{{"command", "orca_useroffline"}}.dump(); }

std::string InlongCloudServiceAgent::build_login_info()
{
    json tree;
    {
        std::lock_guard<std::mutex> lock(session_mutex);
...
std::string InlongCloudServiceAgent::get_access_token() const
{
    std::lock_guard<std::mutex> lock(session_mutex);
    return session.access_token;
}

std::string InlongCloudServiceAgent::get_refresh_token() const
{
    std::lock_guard<std::mutex> lock(session_mutex);
    return session.refresh_token;
}

bool InlongCloudServiceAgent::ensure_token_fresh(const std::string& reason) { return refresh_if_expiring(TOKEN_REFRESH_SKEW, reason); }

// ============================================================================
// ICloudServiceAgent - Server Connectivity
// ============================================================================

int InlongCloudServiceAgent::connect_server()
{
    std::string response;
    unsigned int http_code = 0;
    int result             = http_get(INLONG_HEALTH_PATH, &response, &http_code);
...
bool InlongCloudServiceAgent::is_server_connected()
{
    std::lock_guard<std::recursive_mutex> lock(state_mutex);
    return is_connected;
}

int InlongCloudServiceAgent::refresh_connection() { return connect_server(); }

int InlongCloudServiceAgent::start_subscribe(std::string module)
{
    (void) module;
    return BAMBU_NETWORK_SUCCESS;
}

int InlongCloudServiceAgent::stop_subscribe(std::string module)
{
    (void) module;
    return BAMBU_NETWORK_SUCCESS;
}

int InlongCloudServiceAgent::add_subscribe(std::vector<std::string> dev_list)
{
    (void) dev_list;
    return BAMBU_NETWORK_SUCCESS;
}

int InlongCloudServiceAgent::del_subscribe(std::vector<std::string> dev_list)
{
    (void) dev_list;
    return BAMBU_NETWORK_SUCCESS;
}

void InlongCloudServiceAgent::enable_multi_machine(bool enable)
{
    std::lock_guard<std::recursive_mutex> lock(state_mutex);
...
int InlongCloudServiceAgent::get_user_presets(std::map<std::string, std::map<std::string, std::string>>* user_presets)
{
    if (!user_presets)
        return BAMBU_NETWORK_ERR_INVALID_HANDLE;
    ...
    auto on_success = [&](const SyncPullResponse& resp) {
        ...
        for (const auto& upsert : resp.upserts) {
            ...
            if (value_map.find(INLONG_JSON_KEY_UPDATE_TIME) == value_map.end()) {
                value_map[INLONG_JSON_KEY_UPDATE_TIME] = std::to_string(upsert.updated_time);
            }
...
std::string InlongCloudServiceAgent::request_setting_id(std::string name,
                                                      ...
                                                      unsigned int* http_code)
{
    ...
    BOOST_LOG_TRIVIAL(error) << "InlongCloudServiceAgent: request_setting_id failed - " << result.error_message
...
int InlongCloudServiceAgent::get_setting_list(std::string bundle_version, ProgressFn pro_fn, WasCancelledFn cancel_fn)
{ return get_setting_list2(bundle_version, nullptr, pro_fn, cancel_fn); }

int InlongCloudServiceAgent::get_setting_list2(std::string bundle_version, CheckFn chk_fn, ProgressFn pro_fn, WasCancelledFn cancel_fn)
{
    (void) bundle_version;
    bool success   = false;
    int error_code = 0;
...
int InlongCloudServiceAgent::delete_setting(std::string setting_id)
{
    std::string path = std::string(INLONG_SYNC_DELETE_PATH) + "?resource=" + INLONG_SYNC_PROFILE_TABLE + "&id=" + setting_id;
    std::string response;
    unsigned int http_code = 0;
...
int InlongCloudServiceAgent::sync_pull(std::function<void(const SyncPullResponse&)> on_success,
                                     std::function<void(int http_code, const std::string& error)> on_error)
{
    std::string path = INLONG_SYNC_PULL_PATH;
    ...
    try {
        ...
        if (json.contains("upserts") && json["upserts"].is_array()) {
            for (const auto& item : json["upserts"]) {
                ...
                upsert.updated_time = item.value(INLONG_JSON_KEY_UPDATE_TIME, 0);
                upsert.created_time = item.value(INLONG_JSON_KEY_CREATED_TIME, 0);
...
SyncPushResult InlongCloudServiceAgent::sync_push(const std::string& profile_id,
                                                const std::string& name,
                                                ...
                                                bool force)
{
    ...
    if (body_str.size() > INLONG_SYNC_MAX_PAYLOAD_SIZE) {
        ...
        BOOST_LOG_TRIVIAL(error) << "InlongCloudServiceAgent: sync_push payload too large - " << "size=" << body_str.size() << " bytes, "
                                 << "limit=" << INLONG_SYNC_MAX_PAYLOAD_SIZE << " bytes, " << "profile_id=" << profile_id;
        return result;
    }
    ...
    if (http_code == 409) {
        ...
        try {
            ...
            if (json.is_null()) {
                ...
                result.server_version.updated_time = profile_data.value(INLONG_JSON_KEY_UPDATE_TIME, 0);
            }
        } catch (...) {}
    ...
    try {
        auto json               = nlohmann::json::parse(response);
        result.new_updated_time = json.value(INLONG_JSON_KEY_UPDATE_TIME, 0);
...
void InlongCloudServiceAgent::load_sync_state()
{
    std::lock_guard<std::recursive_mutex> lock(state_mutex);
...
void InlongCloudServiceAgent::save_sync_state()
{
    std::lock_guard<std::recursive_mutex> lock(state_mutex);
...
void InlongCloudServiceAgent::clear_sync_state()
{
    std::lock_guard<std::recursive_mutex> lock(state_mutex);
    sync_state = SyncState{};
...
void InlongCloudServiceAgent::set_session_handler(SessionHandler handler) { session_handler = std::move(handler); }

void InlongCloudServiceAgent::set_on_login_complete_handler(OnLoginCompleteHandler handler)
{ on_login_complete_handler = std::move(handler); }

const InlongCloudServiceAgent::PkceBundle& InlongCloudServiceAgent::pkce()
{
...
void InlongCloudServiceAgent::regenerate_pkce()
{
...
void InlongCloudServiceAgent::update_redirect_uri()
{
...
void InlongCloudServiceAgent::persist_user_secret(const std::string& secret)
{
    if (secret.empty()) {
        clear_user_secret();
        return;
    ...
    if (m_use_encrypted_token_file) {
        ...
        if (key.empty()) {
            BOOST_LOG_TRIVIAL(warning) << "InlongCloudServiceAgent: cannot derive key for user secret file storage";
            return;
        }

        std::string payload;
        if (!aes256gcm_encrypt(secret, key, payload)) {
            BOOST_LOG_TRIVIAL(warning) << "InlongCloudServiceAgent: failed to encrypt user secret for file storage";
            return;
        }
        ...
        if (secret_fallback_path.empty()) {
            BOOST_LOG_TRIVIAL(warning) << "InlongCloudServiceAgent: no user secret storage path available; skipping file persistence";
            return;
        }
        ...
        if (ofs.good()) {
            ...
            if (wxRenameFile(wxString::FromUTF8(tmp_path.c_str()), wxString::FromUTF8(secret_fallback_path.c_str()), true)) {
                ...
                BOOST_LOG_TRIVIAL(warning) << "InlongCloudServiceAgent: failed to atomically replace user secret file";
            }
        } else {
            BOOST_LOG_TRIVIAL(warning) << "InlongCloudServiceAgent: cannot open user secret file for write - " << secret_fallback_path;
        }
    } else {
...
bool InlongCloudServiceAgent::load_user_secret(std::string& out_secret)
{
    out_secret.clear();
                ...
                if (payload.rfind("v2:", 0) == 0) {
                    ...
                    if (delim == std::string::npos) {
                        ...
                        if (computed_hmac.empty() || computed_hmac != lower_stored) {
                            integrity_ok = false;
                            BOOST_LOG_TRIVIAL(warning) << "InlongCloudServiceAgent: user secret integrity check failed (HMAC mismatch)";
                        }
                    }
                }
...
void InlongCloudServiceAgent::clear_user_secret()
{
    wxSecretStore store = wxSecretStore::GetDefault();
    if (store.IsOk()) {
...
bool InlongCloudServiceAgent::should_refresh_locked(std::chrono::seconds skew) const
{
...
bool InlongCloudServiceAgent::decode_jwt_expiry(const std::string& token, std::chrono::system_clock::time_point& out_tp)
{
    out_tp = {};
    if (token.empty())
        return false;
...
RefreshResult InlongCloudServiceAgent::refresh_now(const std::string& refresh_token, const std::string& reason, bool async)
{
...
RefreshResult InlongCloudServiceAgent::refresh_from_storage(const std::string& reason, bool async)
{
    ...
    if (!lock.locked()) {
        ...
        BOOST_LOG_TRIVIAL(warning) << "InlongCloudServiceAgent: token refresh lock unavailable, "
...
bool InlongCloudServiceAgent::refresh_if_expiring(std::chrono::seconds skew, const std::string& reason)
{
...
RefreshResult InlongCloudServiceAgent::refresh_session_with_token(const std::string& refresh_token, const std::string& reason)
{
    ...
    if (http_code >= 200 && http_code < 300) {
        if (session_handler) {
            ...
            // session, so InlongCloudServiceAgent is self-contained without external setup.
            try {
...
bool InlongCloudServiceAgent::set_user_session(const std::string& token,
                                             ...
                                             bool               persist)
{
    ...
    if (!config_dir.empty() && !user_id.empty()) {
        ...
        sync_state_path = (user_dir / INLONG_SYNC_STATE_FILE).string();
        load_sync_state();
    }
...
bool InlongCloudServiceAgent::set_user_session(const json& session_json, bool notify_login)
{
...
void InlongCloudServiceAgent::clear_session()
{
    {
        std::lock_guard<std::mutex> lock(session_mutex);
        session = SessionInfo{};
...
RefreshResult InlongCloudServiceAgent::attempt_refresh_after_unauthorized(const std::string& reason)
{
    RefreshResult r = refresh_from_storage(reason, false);
...
std::map<std::string, std::string> InlongCloudServiceAgent::data_headers()
{
...
bool InlongCloudServiceAgent::resolve_unauthorized(HttpResult& res,
...
int InlongCloudServiceAgent::http_get(const std::string& path, std::string* response_body, unsigned int* http_code)
{
...
int InlongCloudServiceAgent::http_post(const std::string& path, const std::string& body, std::string* response_body, unsigned int* http_code)
{
    std::string url = api_base_url + path;
...
int InlongCloudServiceAgent::http_put(const std::string& path, const std::string& body, std::string* response_body, unsigned int* http_code)
{
    std::string url = api_base_url + path;
...
int InlongCloudServiceAgent::http_delete(const std::string& path, std::string* response_body, unsigned int* http_code)
{
    std::string url = api_base_url + path;
...
bool InlongCloudServiceAgent::http_post_token(const std::string& body,
...
bool InlongCloudServiceAgent::http_post_auth(const std::string& path,
...
void InlongCloudServiceAgent::compute_fallback_path()
{
    if (!secret_fallback_path.empty())
        return;
    ...
    if (wxTheApp == nullptr)
        return;
    wxFileName fallback(wxStandardPaths::Get().GetUserDataDir(), "inlong_refresh_token.sec");
    fallback.Normalize();
    secret_fallback_path = fallback.GetFullPath().ToStdString();
}
...
std::string InlongCloudServiceAgent::map_to_json(const std::map<std::string, std::string>& map)
{
    nlohmann::json j;
...
void InlongCloudServiceAgent::json_to_map(const std::string& json, std::map<std::string, std::string>& map)
{
    try {
...
void InlongCloudServiceAgent::invoke_server_connected_callback(int return_code, int reason_code)
{
    ...
    if (callback) {
        CloudEvent event{INLONG_CLOUD_PROVIDER};
        if (queue_fn) {
...
void InlongCloudServiceAgent::invoke_http_error_callback(unsigned http_code, const std::string& http_body)
{
...
int InlongCloudServiceAgent::set_on_server_connected_fn(AppOnServerConnectedFn fn)
{
    std::lock_guard<std::recursive_mutex> lock(state_mutex);
...
int InlongCloudServiceAgent::set_on_http_error_fn(AppOnHttpErrorFn fn)
{
    std::lock_guard<std::recursive_mutex> lock(state_mutex);
    on_http_error_fn = fn;
    return BAMBU_NETWORK_SUCCESS;
...
int InlongCloudServiceAgent::set_get_country_code_fn(GetCountryCodeFn fn)
{
    std::lock_guard<std::recursive_mutex> lock(state_mutex);
...
int InlongCloudServiceAgent::set_queue_on_main_fn(QueueOnMainFn fn)
{
    std::lock_guard<std::recursive_mutex> lock(state_mutex);
    queue_on_main_fn = fn;
    return BAMBU_NETWORK_SUCCESS;
...
int InlongCloudServiceAgent::get_my_message(int type, int after, int limit, unsigned int* http_code, std::string* http_body)
{
...
int InlongCloudServiceAgent::check_user_task_report(int* task_id, bool* printable)
{
...
int InlongCloudServiceAgent::get_user_print_info(unsigned int* http_code, std::string* http_body)
{
...
int InlongCloudServiceAgent::get_user_tasks(TaskQueryParams params, std::string* http_body)
{
...
int InlongCloudServiceAgent::get_printer_firmware(std::string dev_id, unsigned* http_code, std::string* http_body)
{
...
int InlongCloudServiceAgent::get_task_plate_index(std::string task_id, int* plate_index)
{
...
int InlongCloudServiceAgent::get_user_info(int* identifier)
{
...
int InlongCloudServiceAgent::get_subtask_info(std::string subtask_id, std::string* task_json, unsigned int* http_code, std::string* http_body)
{
...
int InlongCloudServiceAgent::get_slice_info(std::string project_id, std::string profile_id, int plate_index, std::string* slice_json)
{
...
int InlongCloudServiceAgent::query_bind_status(std::vector<std::string> query_list, unsigned int* http_code, std::string* http_body)
{
...
int InlongCloudServiceAgent::modify_printer_name(std::string dev_id, std::string dev_name)
{
    BOOST_LOG_TRIVIAL(debug) << "InlongCloudServiceAgent: modify_printer_name (stub)";
    return BAMBU_NETWORK_SUCCESS;
}

int InlongCloudServiceAgent::get_camera_url(std::string dev_id, std::function<void(std::string)> callback)
{
...
int InlongCloudServiceAgent::get_design_staffpick(int offset, int limit, std::function<void(std::string)> callback)
{
...
int InlongCloudServiceAgent::start_publish(PublishParams params, OnUpdateStatusFn update_fn, WasCancelledFn cancel_fn, std::string* out)
{
    BOOST_LOG_TRIVIAL(debug) << "InlongCloudServiceAgent: start_publish (stub)";
    return BAMBU_NETWORK_SUCCESS;
}

int InlongCloudServiceAgent::get_model_publish_url(std::string* url)
{
...
int InlongCloudServiceAgent::get_subtask(BBLModelTask* task, OnGetSubTaskFn getsub_fn)
{
    BOOST_LOG_TRIVIAL(debug) << "InlongCloudServiceAgent: get_subtask (stub)";
    return BAMBU_NETWORK_SUCCESS;
}

int InlongCloudServiceAgent::get_model_mall_home_url(std::string* url)
{
...
int InlongCloudServiceAgent::get_model_mall_detail_url(std::string* url, std::string id)
{
...
int InlongCloudServiceAgent::get_my_profile(std::string token, unsigned int* http_code, std::string* http_body)
{
...
int InlongCloudServiceAgent::get_my_token(std::string ticket, unsigned int* http_code, std::string* http_body)
{
...
int InlongCloudServiceAgent::track_enable(bool enable)
{
    std::lock_guard<std::recursive_mutex> lock(state_mutex);
    enable_track = enable;
    return BAMBU_NETWORK_SUCCESS;
...
int InlongCloudServiceAgent::track_remove_files()
{
    BOOST_LOG_TRIVIAL(debug) << "InlongCloudServiceAgent: track_remove_files (stub)";
    return BAMBU_NETWORK_SUCCESS;
}

int InlongCloudServiceAgent::track_event(std::string evt_key, std::string content)
{
    BOOST_LOG_TRIVIAL(debug) << "InlongCloudServiceAgent: track_event (stub) - " << evt_key;
    return BAMBU_NETWORK_SUCCESS;
}

int InlongCloudServiceAgent::track_header(std::string header)
{
    BOOST_LOG_TRIVIAL(debug) << "InlongCloudServiceAgent: track_header (stub)";
    return BAMBU_NETWORK_SUCCESS;
}

int InlongCloudServiceAgent::track_update_property(std::string name, std::string value, std::string type)
{
    BOOST_LOG_TRIVIAL(debug) << "InlongCloudServiceAgent: track_update_property (stub) - " << name;
    return BAMBU_NETWORK_SUCCESS;
}

int InlongCloudServiceAgent::track_get_property(std::string name, std::string& value, std::string type)
{
...
bool InlongCloudServiceAgent::get_track_enable()
{
    std::lock_guard<std::recursive_mutex> lock(state_mutex);
    return enable_track;
}

int InlongCloudServiceAgent::put_model_mall_rating(
...
int InlongCloudServiceAgent::get_oss_config(std::string& config, std::string country_code, unsigned int& http_code, std::string& http_error)
{
...
int InlongCloudServiceAgent::put_rating_picture_oss(
...
int InlongCloudServiceAgent::get_model_mall_rating_result(int job_id,
...
std::string InlongCloudServiceAgent::get_cloud_service_host() { return api_base_url; }

std::string InlongCloudServiceAgent::get_cloud_login_url(const std::string& language)
{
    std::string url = cloud_base_url + INLONG_CLOUD_LOGIN_PATH;
    if (!language.empty()) {
        url += "?lang=" + language;
    }
    return url;
...
int InlongCloudServiceAgent::get_mw_user_preference(std::function<void(std::string)> callback)
{
...
int InlongCloudServiceAgent::get_mw_user_4ulist(int seed, int limit, std::function<void(std::string)> callback)
{
...
std::string InlongCloudServiceAgent::get_version() { return "InlongCloudServiceAgent 1.0.0"; }

// ============================================================================
// Bundle Subscription Implementation
// ============================================================================

bool InlongCloudServiceAgent::unsubscribe_bundle(const std::string& bundle_id)
{
    std::string path = std::string(INLONG_SUBSCRIPTIONS_PATH) + "/" + bundle_id;
    std::string response;
    unsigned int http_code = 0;
...
std::string InlongCloudServiceAgent::get_bundle_url(const std::string& bundle_id) const
{ return cloud_base_url + "/app/bundles/subscribed-bundles/" + bundle_id; }

int InlongCloudServiceAgent::get_subscribed_bundles(std::vector<std::pair<std::string, std::string>>* bundles,
...
int InlongCloudServiceAgent::get_shared_bundle(const std::string& bundle_id,
                                             ...
                                             BundleMetadata* bundle_metadata)
{
    ...
    try {
        ...
        for (auto& preset_object : json["shared_profiles"]) {
            ...
            if (preset_object.contains("updated_time") && value_map.find(INLONG_JSON_KEY_UPDATE_TIME) == value_map.end()) {
                value_map[INLONG_JSON_KEY_UPDATE_TIME] = preset_object["updated_time"].dump();
            }
...
int InlongCloudServiceAgent::fetch_subscribed_manifests_into_descriptors(std::vector<PluginDescriptor>& descriptors,
                                                            ...
                                                            std::vector<std::string>& unauthorized)
{
    ...
    const std::string path = std::string(INLONG_SUBSCRIBED_PLUGINS) + "?include=changelog";
    int result = http_get(path, &response_body, &http_code);
    ...
    try {
        ...
        for (const auto& item : root["data"]) {
            ...
            // plugin's Python header once installed. Cloud-only rows show a "View on Inlong Cloud" link.
...
int InlongCloudServiceAgent::fetch_mine_manifests_into_descriptors(std::vector<PluginDescriptor>& descriptors)
{
    ...
    while (true) {
        ...
        const std::string path = std::string(INLONG_PLUGINS_MINE) + "?limit=" + std::to_string(PAGE_LIMIT) +
...
int InlongCloudServiceAgent::get_plugin_download_url(const std::string& uuid,
                                                   ...
                                                   std::vector<std::string>& unauthorized)
{
    ...
    std::string path       = INLONG_PLUGIN_DOWNLOAD_URL;
    if (!os.empty())
...
std::string InlongCloudServiceAgent::get_plugin_url(const std::string& sharing_token) const
{ return cloud_base_url + "/p/" + Http::url_encode(sharing_token); }

int InlongCloudServiceAgent::subscribe_plugin(const std::string& plugin_uuid)
{
    ...
    const int result = http_post(INLONG_SUBSCRIBED_PLUGINS, request_body, &response_body, &http_code);
...
int InlongCloudServiceAgent::unsubscribe_plugins(const std::vector<std::string>& plugin_uuids)
{
    ...
    const std::string path = std::string(INLONG_UNSUBSCRIBE_PLUGINS) + "?ids=" + query;
                                                   ...
                                                   std::unordered_map<std::string, std::vector<PluginChangelog>>& changelog)
{
    ...
    for (const std::string& uuid : uuids) {
        ...
        const std::string path = std::string(INLONG_PLUGINS_BASE) + "/" + Http::url_encode(uuid) + "/changelog?limit=100";
        std::string response_body;
        unsigned int http_code = 0;
...
std::string InlongCloudServiceAgent::token_lock_path() const
{
    if (config_dir.empty())
        return {};
    wxFileName lock(wxString::FromUTF8(config_dir.c_str()), "inlong_refresh_token.lock");
    lock.Normalize();
