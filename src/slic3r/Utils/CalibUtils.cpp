...
void get_default_k_n_value(const std::string &filament_id, float &k, float &n)
{
    ...
    const std::string printer_filament_id = agent ? agent->from_inlong_filament_id(filament_id) : filament_id;
...
bool CalibUtils::calib_flowrate(int pass, const CalibInfo &calib_info, wxString &error_message)
{
    ...
    for (auto _obj : model.objects) {
        ...
        _obj->config.set_key_value("filter_out_gap_fill", new ConfigOptionFloat(0));  // InlongSlicer parameter
