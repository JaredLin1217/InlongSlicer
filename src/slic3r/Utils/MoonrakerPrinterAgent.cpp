...
std::string MoonrakerPrinterAgent::map_filament_type_to_generic_id(const std::string& filament_type)
{
    ...
    // Normalize reported material names (trimmed, uppercased) to an InlongFilamentLibrary
    ...
    while (!ws_stop.load() && retry_count < max_retries) {
        ...
        try {
            ...
            ws.set_option(websocket::stream_base::decorator([&](websocket::request_type& req) {
                req.set(http::field::user_agent, "InlongSlicer");
                if (!api_key.empty()) {
            ...
            identify["params"]["client_name"] = "InlongSlicer";
            identify["params"]["version"]     = MoonrakerPrinterAgent_VERSION;
            identify["params"]["type"]        = "agent";
            identify["params"]["url"]         = "https://github.com/JaredLin1217/InlongSlicer";
            identify["id"]                    = 0;
