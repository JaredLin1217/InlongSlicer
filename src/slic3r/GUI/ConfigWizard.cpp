...
BundleMap BundleMap::load()
{
    ...
    // Inlong: add the custom bundle as the default and load it through the
    // cache-aware vendor path introduced upstream.
    {
        const bool from_rsrc = ! is_vendor_installed(PresetBundle::INLONG_DEFAULT_BUNDLE);
        Bundle bbl_bundle;
        if (bbl_bundle.load(from_rsrc ? rsrc_vendor_dir : vendor_dir, PresetBundle::INLONG_DEFAULT_BUNDLE, from_rsrc, true))
