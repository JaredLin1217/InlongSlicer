...
BundleMap BundleMap::load()
{
    ...
    auto inlong_bundle_path = (vendor_dir / PresetBundle::INLONG_DEFAULT_BUNDLE).replace_extension(".json");
    auto inlong_bundle_rsrc = false;
    if (!boost::filesystem::exists(inlong_bundle_path)) {
        inlong_bundle_path = (rsrc_vendor_dir / PresetBundle::INLONG_DEFAULT_BUNDLE).replace_extension(".json");
        inlong_bundle_rsrc = true;
    }
    {
        Bundle bbl_bundle;
        if (bbl_bundle.load(std::move(inlong_bundle_path), inlong_bundle_rsrc, true))
            res.emplace(PresetBundle::INLONG_DEFAULT_BUNDLE, std::move(bbl_bundle));
    }
...
{
    //Inlong: add custom as default
    auto it = find(PresetBundle::INLONG_DEFAULT_BUNDLE);
    if (it == end()) {
        throw Slic3r::RuntimeError("ConfigWizard: Internal error in BundleMap: INLONG_DEFAULT_BUNDLE not loaded");
    }
...
#endif
            ) % SLIC3R_APP_FULL_NAME).str()), _L("Welcome"))
...
void PagePrinters::set_run_reason(ConfigWizard::RunReason run_reason)
{
    ...
    if (is_primary_printer_page
        ...
        && printer_pickers[0]->vendor_id == PresetBundle::INLONG_DEFAULT_BUNDLE) {
...
ConfigWizardIndex::ConfigWizardIndex(wxWindow *parent)
    : wxPanel(parent)
    , bg(ScalableBitmap(parent, "InlongSlicer_192px_transparent.png", 192))
...
void ConfigWizard::priv::create_3rdparty_pages()
{
    for (const auto &pair : bundles) {
        ...
        //Inlong: add custom as default
        if (vendor->id == PresetBundle::INLONG_DEFAULT_BUNDLE) { continue; }
...
ConfigWizard::ConfigWizard(wxWindow *parent)
    : DPIDialog(parent, wxID_ANY, wxString(SLIC3R_APP_FULL_NAME) + " - " + _(name()), wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE /*| wxRESIZE_BORDER*/)
    , p(new priv(this))
{
