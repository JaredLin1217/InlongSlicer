                                             ...
                                             const std::vector<std::string>& physical_types)
{
    ...
    if (auto* filament_id_opt = project_config.option<ConfigOptionStrings>("filament_id")) {
        if (source_config_idx < filament_id_opt->values.size()) {
            ...
            const std::string& inlong_filament_id = filament_id_opt->values[source_config_idx];
            const std::string  printer_filament_id = agent ? agent->from_inlong_filament_id(inlong_filament_id) : inlong_filament_id;
...
std::string decompose_basic_filament_id(const std::string& basic_type)
{
    ...
    auto* agent = wxGetApp().getAgent();
    return agent ? agent->to_inlong_filament_id(printer_filament_id) : printer_filament_id;
}
...
void set_created_standard_component_metadata(size_t config_idx, const DecomposeOfficialComponent& component)
{
    ...
    auto* agent = wxGetApp().getAgent();
    const std::string printer_filament_id = agent ? agent->from_inlong_filament_id(component.filament_id) : component.filament_id;
    ...
    const wxColour& fallback)
{
    ...
    for (const std::string& candidate : candidate_names) {
        for (const auto& item : root["data"]) {
            ...
            auto* agent = wxGetApp().getAgent();
            result.filament_id = agent ? agent->to_inlong_filament_id(fila_id) : fila_id;
            return result;
        }
    }
    return result;
