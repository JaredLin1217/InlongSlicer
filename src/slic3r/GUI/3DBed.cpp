...
// INLONG make bed colors accessable for 2D bed
...
void Bed3D::Axes::render()
{
    ...
    if (!m_arrow.is_initialized())
        ...
        m_arrow.init_from(smooth_cylinder(16, /*Radius*/ m_stem_length / 75.f, m_stem_length)); // INLONG use simple cylinder and scale thickness depends on length

    GLShaderProgram* shader = wxGetApp().get_shader("flat"); // INLONG dont use shading to get closer color tone
    if (shader == nullptr)
        return;
...
void Bed3D::render_default(bool bottom, const Transform3d& view_matrix, const Transform3d& projection_matrix)
{
    ...
    if (shader != nullptr) {
        ...
        if (m_model.get_filename().empty() && !bottom) {
            // draw background
            glsafe(::glDepthMask(GL_FALSE));
            ColorRGBA color = m_is_dark ? DEFAULT_MODEL_COLOR_DARK : DEFAULT_MODEL_COLOR;   // INLONG add dark mode support
            color = ColorRGBA(color[0] * 0.8f, color[1] * 0.8f,color[2] * 0.8f, color[3]);  // INLONG shift color a darker tone to fix difference between flat / gouraud_light shader
