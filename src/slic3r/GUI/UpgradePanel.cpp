...
void UpgradePanel::update(MachineObject *obj)
{
    ...
    if (m_obj && m_show_consistency_hint) {
        if (m_obj->upgrade_consistency_request) {
            ...
            consistency_dlg->update_text(_L("The firmware version is abnormal. Repairing and updating are required before printing. Do you want to update now\? You can also update later on the printer or update next time you start Inlong Slicer."));
