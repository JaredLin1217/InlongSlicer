...
void NotificationManager::InlongSyncConflictNotification::render_text(ImGuiWrapper& imgui,
	const float win_size_x, const float win_size_y,
	const float win_pos_x, const float win_pos_y)
{
	float x_offset = m_left_indentation;
...
{
	close_notification_of_type(NotificationType::InlongSharedProfilesAvailable);
	NotificationData data{ NotificationType::InlongSharedProfilesAvailable, NotificationLevel::RegularNotificationLevel, 0,
...
void NotificationManager::push_inlong_sync_conflict_notification(const std::string& text,
	int conflict_code,
	std::function<bool(wxEvtHandler*)> pull_callback,
	std::function<bool(wxEvtHandler*)> force_push_callback)
{
	close_notification_of_type(NotificationType::InlongSyncConflict);
