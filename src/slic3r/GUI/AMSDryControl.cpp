...
void AMSDryCtrWin::create()
{
    ...
    std::string icon_path = (boost::format("%1%/images/InlongSlicerTitle.ico") % resources_dir()).str(); // Orca: app title icon
...
void AMSDryCtrWin::update_filament_guide_info(DevAms* dev_ams)
{
    ...
    const std::string pla_filament_id = agent ? agent->to_inlong_filament_id("GFA00") : std::string("GFA00");
...
int AMSDryCtrWin::update_filament_list(DevAms* dev_ams, MachineObject* obj)
{
    ...
    auto* agent = wxGetApp().getAgent();
    std::string default_filament_id = agent ? agent->to_inlong_filament_id("GFA00") : std::string("GFA00");   // compared against m_tray_ids[i].filament_id (our OF ids) below
