    ...
    wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE)
{
    ...
    auto hash_url = "https://github.com/JaredLin1217/InlongSlicer/commit/" + wxString(build_commit_hash);
    ...
    log_pack_btn->Bind(wxEVT_BUTTON, [this](wxCommandEvent &e) {
        auto data_dir   = boost::filesystem::path(Slic3r::data_dir());
        ExportAsZip({wxString((data_dir / "log").string())}, "InlongSlicer_Logs_" + GetTimestamp());
    });
...
wxString TroubleshootDialog::GetPackageType()
{
    ...
    if (path.Contains("InlongSlicer\\build"))
        return "Local Build";
    ...
    wxString path = wxStandardPaths::Get().GetExecutablePath();
    if (path.Contains("InlongSlicer/build")) return "Local Build";
    ...
    if (path.StartsWith("/Volumes/InlongSlicer")) // running from .dmg
        return "Temporary";
...
void TroubleshootDialog::PackAll()
{
    ...
    ExportAsZip(include_zip, "InlongSlicer_PackedDebugInfo_" + GetTimestamp());
}
