...
wxString WipingDialog::BuildTextObjStr(bool multi_language)
{
    ...
    if (multi_language) {
        auto_flush_tip = wxString::Format(_L("%s would re-calculate your flushing volumes everytime the filaments color changed or filaments changed. You could disable the auto-calculate in %s > Preferences"), wxString(SLIC3R_APP_FULL_NAME), wxString(SLIC3R_APP_FULL_NAME));
        ...
        auto_flush_tip = wxString::Format("%s would re-calculate your flushing volumes everytime the filaments color changed or filaments changed. You could disable the auto-calculate in %s > Preferences", wxString(SLIC3R_APP_FULL_NAME), wxString(SLIC3R_APP_FULL_NAME));
