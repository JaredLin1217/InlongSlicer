...
GuideFrame::GuideFrame(GUI_App *pGUI, long style)
    : DPIDialog((wxWindow *) (pGUI->mainframe), wxID_ANY, "InlongSlicer", wxDefaultPosition, wxDefaultSize, style),
	m_appconfig_new()
{
...
void GuideFrame::init_guide_paths()
{
    ...
    if (boost::filesystem::exists(vendor_dir)) {
        for (const auto& entry : boost::filesystem::directory_iterator(vendor_dir)) {
            if (!boost::filesystem::is_directory(entry) &&
                ...
                !boost::iequals(entry.path().stem().string(), PresetBundle::INLONG_FILAMENT_LIBRARY)) {
    ...
    auto lib_json = boost::filesystem::path(PresetBundle::INLONG_FILAMENT_LIBRARY).replace_extension(".json");
    m_InlongFilaLibPath = boost::filesystem::exists(vendor_dir / lib_json)
        ? (vendor_dir      / PresetBundle::INLONG_FILAMENT_LIBRARY).string()
        : (rsrc_vendor_dir / PresetBundle::INLONG_FILAMENT_LIBRARY).string();
}
...
bool GuideFrame::BuildProfileJson(const PresetBundle& bundle, bool require_all_resource_vendors)
{
    try {
        ...
        if (require_all_resource_vendors) {
            ...
            // InlongFilamentLibrary+Custom). Fall back so the slow path reads both dirs.
            try {
