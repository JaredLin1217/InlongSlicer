        ...
        if (get_obj()) {
            if (state_cloud_task == 0 && m_job_id == get_obj()->profile_id_) {
                ...
                if (state_device == 0) {
                    ...
                    dc.SetTextForeground(wxColour(214, 108, 71));
                    DrawTextWithEllipsis(dc, get_state_device(), FromDIP(DEVICE_LEFT_PRO_INFO), left);
                }
                else if (state_device == 2)
                {
                    ...
                    if (obj_->get_curr_stage() == _L("Printing") && obj_->subtask_) {
                        ...
                        dc.SetPen(wxPen(wxColour(214, 108, 71)));
                        dc.SetBrush(wxBrush(wxColour(214, 108, 71)));
    ...
    : wxPanel(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxTAB_TRAVERSAL)
{
    ...
    page_sizer->Add(m_main_panel, 1, wxALL | wxEXPAND, FromDIP(10)); // INLONG match margin with other tabs
