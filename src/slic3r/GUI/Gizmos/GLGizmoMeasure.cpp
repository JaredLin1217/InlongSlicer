...
void GLGizmoMeasure::render_debug_dialog()
{
    auto add_feature_data = [this](const SelectedFeatures::Item& item) {
        ...
        add_strings_row_to_table(*m_imgui, "Type", ImGuiWrapper::COL_INLONG, text, ImGui::GetStyleColorVec4(ImGuiCol_Text));
        switch (item.feature->get_type())
        {
        case Measure::SurfaceFeatureType::Point:
        {
            add_strings_row_to_table(*m_imgui, "m_pt1", ImGuiWrapper::COL_INLONG, format_vec3(item.feature->get_point()), ImGui::GetStyleColorVec4(ImGuiCol_Text));
            break;
        }
        case Measure::SurfaceFeatureType::Edge:
        {
            auto [from, to] = item.feature->get_edge();
            add_strings_row_to_table(*m_imgui, "m_pt1", ImGuiWrapper::COL_INLONG, format_vec3(from), ImGui::GetStyleColorVec4(ImGuiCol_Text));
            add_strings_row_to_table(*m_imgui, "m_pt2", ImGuiWrapper::COL_INLONG, format_vec3(to), ImGui::GetStyleColorVec4(ImGuiCol_Text));
            break;
        }
        case Measure::SurfaceFeatureType::Plane:
        {
            auto [idx, normal, origin] = item.feature->get_plane();
            add_strings_row_to_table(*m_imgui, "m_pt1", ImGuiWrapper::COL_INLONG, format_vec3(normal), ImGui::GetStyleColorVec4(ImGuiCol_Text));
            add_strings_row_to_table(*m_imgui, "m_pt2", ImGuiWrapper::COL_INLONG, format_vec3(origin), ImGui::GetStyleColorVec4(ImGuiCol_Text));
            add_strings_row_to_table(*m_imgui, "m_value", ImGuiWrapper::COL_INLONG, format_double(idx), ImGui::GetStyleColorVec4(ImGuiCol_Text));
            break;
        }
        case Measure::SurfaceFeatureType::Circle:
        {
            ...
            add_strings_row_to_table(*m_imgui, "m_pt1", ImGuiWrapper::COL_INLONG, format_vec3(center), ImGui::GetStyleColorVec4(ImGuiCol_Text));
            add_strings_row_to_table(*m_imgui, "m_pt2", ImGuiWrapper::COL_INLONG, format_vec3(normal), ImGui::GetStyleColorVec4(ImGuiCol_Text));
            add_strings_row_to_table(*m_imgui, "m_value", ImGuiWrapper::COL_INLONG, format_double(radius), ImGui::GetStyleColorVec4(ImGuiCol_Text));
            break;
        }
        }
        std::optional<Vec3d> extra_point = item.feature->get_extra_point();
        if (extra_point.has_value())
            add_strings_row_to_table(*m_imgui, "m_pt3", ImGuiWrapper::COL_INLONG, format_vec3(*extra_point), ImGui::GetStyleColorVec4(ImGuiCol_Text));
    };
    ...
    if (ImGui::BeginTable("Mode", 2)) {
        ...
        add_strings_row_to_table(*m_imgui, "Mode", ImGuiWrapper::COL_INLONG, txt, ImGui::GetStyleColorVec4(ImGuiCol_Text));
        ImGui::EndTable();
    }

    ImGui::Separator();
    if (ImGui::BeginTable("Hover", 2)) {
        add_strings_row_to_table(*m_imgui, "Hover id", ImGuiWrapper::COL_INLONG, std::to_string(m_hover_id), ImGui::GetStyleColorVec4(ImGuiCol_Text));
        const std::string txt = m_curr_feature.has_value() ? surface_feature_type_as_string(m_curr_feature->get_type()) : "None";
        add_strings_row_to_table(*m_imgui, "Current feature", ImGuiWrapper::COL_INLONG, txt, ImGui::GetStyleColorVec4(ImGuiCol_Text));
        ImGui::EndTable();
    }
    ...
    if (!m_selected_features.first.feature.has_value() && !m_selected_features.second.feature.has_value())
        ...
        if (m_selected_features.first.feature.has_value()) {
            m_imgui->text_colored(ImGuiWrapper::COL_INLONG, "Selection 1");
        ...
        if (m_selected_features.second.feature.has_value()) {
            m_imgui->text_colored(ImGuiWrapper::COL_INLONG, "Selection 2");
...
void GLGizmoMeasure::show_distance_xyz_ui()
{
    ...
    auto add_edit_distance_xyz_box = [this](Vec3d &distance) {
        ...
        //{
            ...
            ImGui::TableNextRow();
            ImGui::TableSetColumnIndex(0);
            m_imgui->text_colored(ImGuiWrapper::to_ImVec4(ColorRGBA::X()), "X:"); // INLONG match axis color
            ImGui::TableSetColumnIndex(1);
            ImGui::PushItemWidth(m_input_size_max);
            m_imgui->disabled_begin(volume || !m_can_set_xyz_distance); // INLONG disable only input box othervise axis colors rendered dimmed
            ImGui::BBLInputDouble("##measure_distance_x", &m_buffered_distance[0], 0.0f, 0.0f, "%.2f");
            m_imgui->disabled_end();

            ImGui::TableNextRow();
            ImGui::TableSetColumnIndex(0);
            m_imgui->text_colored(ImGuiWrapper::to_ImVec4(ColorRGBA::Y()), "Y:"); // INLONG match axis color
            ImGui::TableSetColumnIndex(1);
            m_imgui->disabled_begin(volume || !m_can_set_xyz_distance); // INLONG disable only input box othervise axis colors rendered dimmed
            ...
            ImGui::TableNextRow();
            ImGui::TableSetColumnIndex(0);
            m_imgui->text_colored(ImGuiWrapper::to_ImVec4(ColorRGBA::Z()), "Z:"); // INLONG match axis color
            ImGui::TableSetColumnIndex(1);
            m_imgui->disabled_begin(volume || !(m_same_model_object && m_can_set_xyz_distance)); // INLONG disable only input box othervise axis colors rendered dimmed
    ...
    if (ImGui::BeginTable("Measure", 4)) {
        if (m_selected_features.second.feature.has_value()) {
            ...
            if (measure.angle.has_value() && m_measure_mode == EMeasureMode::ONLY_MEASURE)
                ...
                add_measure_row_to_table(_u8L("Angle"), ImGuiWrapper::COL_INLONG, format_double(Geometry::rad2deg(measure.angle->angle)) + "°",
            ...
            if (measure.distance_infinite.has_value() && m_measure_mode == EMeasureMode::ONLY_MEASURE) {
                ...
                add_measure_row_to_table(show_strict ? _u8L("Perpendicular distance") : _u8L("Distance"), ImGuiWrapper::COL_INLONG, format_double(distance) + m_units,
                                            ImGui::GetStyleColorVec4(ImGuiCol_Text));
                ++measure_row_count;
                ImGui::PopID();
            }
            if (show_strict &&
                ...
                add_measure_row_to_table(_u8L("Direct distance"), ImGuiWrapper::COL_INLONG, format_double(distance) + m_units, ImGui::GetStyleColorVec4(ImGuiCol_Text));
                ++measure_row_count;
                ImGui::PopID();
            }
            if (measure.distance_xyz.has_value() && m_measure_mode == EMeasureMode::ONLY_MEASURE) {
                ...
                if (measure.distance_xyz->norm() > EPSILON) {
                    ...
                    add_measure_row_to_table(_u8L("Distance XYZ"), ImGuiWrapper::COL_INLONG, format_vec3(distance), ImGui::GetStyleColorVec4(ImGuiCol_Text));
                    ++measure_row_count;
                    ImGui::PopID();
                }
            }
        ...
        /*for (unsigned int i = measure_row_count; i < max_measure_row_count; ++i) {
            add_strings_row_to_table(*m_imgui, " ", ImGuiWrapper::COL_INLONG, " ", ImGui::GetStyleColorVec4(ImGuiCol_Text));
        }*/
        ImGui::EndTable();
    }
}
...
void GLGizmoMeasure::show_face_face_assembly_common() {
    if (m_measure_mode == EMeasureMode::ONLY_ASSEMBLY && m_hit_different_volumes.size() == 2 &&
        ...
        m_imgui->disabled_begin(!(action.can_set_to_center_coincidence));
        {
            ...
            ImGui::PushStyleColor(ImGuiCol_Button, ImGuiWrapper::COL_INLONG);
            ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImGuiWrapper::COL_ORCA_HOVER);
            ImGui::PushStyleColor(ImGuiCol_ButtonActive, ImVec4(185 / 255.0f, 87 / 255.0f, 58 / 255.0f, 1.00f));
