                            ...
                            std::function<void(const wxString &)> link_callback = nullptr)
{
    ...
    if (msg.Contains("<tr>")) {
        ...
        if (has_code_excerpts) {
            ...
            wxClientDC dc(parent);
            dc.SetFont(font); // INLONG: without this it calculates a larger size
    ...
    if (!link_text.IsEmpty() && link_callback) {
        msg_escaped += "<span><a href=\"#\" style=\"color:rgb(214, 108, 71); text-decoration:underline;\">" + std::string(link_text.ToUTF8().data()) + "</a></span>";
    }
    ...
    , msg(temp_msg)
{
	...
	logo->SetBitmap(create_scaled_bitmap("InlongSlicer_192px_grayscale.png", this, has_code_excerpts ? 48 : /*1*/64));
