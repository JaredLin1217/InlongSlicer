    ...
    if (m_type == Preset::TYPE_FILAMENT) {
        ...
        clr_picker->Bind(wxEVT_BUTTON, [this](wxCommandEvent& e) {
            ...
            if (is_official) {
                ...
                if (selected_preset)
                    fila_id = wxString::FromUTF8(agent ? agent->from_inlong_filament_id(selected_preset->filament_id) : selected_preset->filament_id);
...
void PlaterPresetComboBox::update()
{
    ...
    std::map<wxString, std::string> preset_aliases; // INLONG
    ...
    for (size_t i = presets.front().is_visible ? 0 : m_collection->num_default_presets(); i < presets.size(); ++i)
    {
        ...
        preset_aliases[name] = get_preset_name(preset).utf8_string(); // INLONG
...
#endif
            // INLONG allow caching vendor and type values for all presets instead just system ones
            // if (preset.is_system) { 
        ...
        if (!preset.is_compatible) {
            ...
            if (m_type == Preset::TYPE_PRINTER) {
                ...
                // INLONG: Make system printer presets display the dirty "*" prefix when edited.
                ...
                if (system_printer_models.count(printer_model) == 0) {
                    preset_aliases[name] = name.utf8_string(); // INLONG
                    ...
                    preset_aliases.erase(alternate_name);  // INLONG: do this to aliases too
    ...
    auto  add_presets       = [this, &preset_descriptions, &filament_orders, &preset_filament_vendors, &first_vendors, &preset_filament_types, &preset_aliases, &preset_bundle_ids, &preset_bundle_names, &first_types, &selected_in_ams]
        ...
        if (!presets.empty()) {
            ...
            if (m_type == Preset::TYPE_FILAMENT || m_type == Preset::TYPE_PRINTER) {
                ...
                // INLONG add sorting support for vendor / type for user presets. also non grouped items
                ...
                for (auto it : list) {
                    // INLONG add sorting support for vendor / type for user presets
    ...
    // INLONG: the by_type/by_vendor grouping is derived from filament-only attributes
    ...
    // INLONG: add bundle presets with sub-dropdown grouping for filament and printer
...
void TabPresetComboBox::update()
{
    ...
    for (size_t i = presets.front().is_visible ? 0 : m_collection->num_default_presets(); i < presets.size(); ++i)
    {
        ...
        // INLONG: Track bundle names for bundled presets
        if (preset.is_from_bundle()) {
             m_preset_bundle->bundles.ReadLock();
            auto bundle_it = m_preset_bundle->bundles.m_bundles.find(preset.bundle_id);
            if (bundle_it != m_preset_bundle->bundles.m_bundles.end()) {
    ...
    // INLONG: add bundle presets with sub-dropdown grouping
