...
void OG_CustomCtrl::OnPaint(wxPaintEvent&)
{
    ...
    wxCoord h_pos = get_title_width() * m_em_unit + 4; // INLONG Align label with group title. StaticLine.cpp uses 18px for icon 5px for spacing. Spacing doesnt scales on messureSize()
    wxCoord v_pos = 0;
    // BBS: new layout
    if (!GetLabel().IsEmpty()) {
...
void OG_CustomCtrl::CtrlLine::render(wxDC& dc, wxCoord h_pos, wxCoord v_pos)
{
    ...
    wxColour blink_color = StateColor::darkModeColorFor("#D66C47");
...
wxCoord OG_CustomCtrl::CtrlLine::draw_text(wxDC &dc, wxPoint pos, const wxString &text, const wxColour *color, int width, bool is_url/* = false*/, bool is_main/* = false*/)
{
    ...
    if (!text.IsEmpty()) {
        ...
        wxColour old_clr = dc.GetTextForeground();
        wxFont old_font = dc.GetFont();
        wxColor clr_url = StateColor::darkModeColorFor("#D66C47");
...
#endif
    wxCoord h_pos = pos.x - ctrl->m_h_gap;  // Inlong: adjust position to the left
    wxCoord v_pos = pos.y;
