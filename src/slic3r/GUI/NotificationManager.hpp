...
//	LoadingFailed,
    ...
    // non-modal; offers install (cloud) / Inlong Cloud search (local) and blocks slicing.
    InlongLocalPluginMissingError,
    InlongCloudPluginMissingError,
    ...
    InlongPluginCapabilityUnavailableError,
...
//	void set_slicing_warning_gray(const std::string& text, bool g);
	...
	void push_inlong_sync_conflict_notification(const std::string& text,
		int conflict_code,
	...
	// open Inlong Cloud). The callback returns true to close the notification, or false to keep it
...
private:
	...
	class InlongSyncConflictNotification : public PopNotification
	{
	public:
		InlongSyncConflictNotification(const NotificationData& n, NotificationIDProvider& id_provider, wxEvtHandler* evt_handler,
			std::function<bool(wxEvtHandler*)> pull_callback,
			std::function<bool(wxEvtHandler*)> force_push_callback,
			int conflict_code)
			: PopNotification(n, id_provider, evt_handler)
