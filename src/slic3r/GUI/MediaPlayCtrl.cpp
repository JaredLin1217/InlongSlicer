...
void MediaPlayCtrl::ToggleStream()
{
    ...
    if (!url.empty() && wxGetApp().app_config->get("not_show_vcamera_stop_prev") != "1") {
        MessageDialog dlg(this->GetParent(), wxString::Format(_L("Another virtual camera is running.\n%s supports only a single virtual camera.\nDo you want to stop this virtual camera?"), wxString(SLIC3R_APP_FULL_NAME)), _L("Warning"),
