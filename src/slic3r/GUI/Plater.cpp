...
bool Sidebar::priv::switch_diameter(bool single)
{
    ...
    const Preset &selected_printer = preset_bundle->printers.get_edited_preset();
    const auto &default_filament_profiles = selected_printer.config.option<ConfigOptionStrings>("default_filament_profile")->values;
    if (!default_filament_profiles.empty() && !Sidebar::should_show_SEMM_buttons()) {
        const int filament_count = std::max(1, preset_bundle->get_printer_extruder_count());
...
void Sidebar::update_all_preset_comboboxes()
{
    ...
    if (preset_bundle.use_bbl_network()) {
        ...
        // INLONG: hide the physical-printer connection button when printer agents are enabled
...
void Sidebar::update_presets(Preset::Type preset_type)
{
    ...
    case Preset::TYPE_PRINTER:
    {
        ...
        const bool is_dual_extruder = extruder_variants->size() == 2 || nozzle_diameter->size() == 2;
        // Agent mode drives the native device tab, so hide the physical-printer
        // connection button while preserving Inlong dual-extruder controls.
...
void Sidebar::sync_ams_list(bool is_from_big_sync_btn)
{
    ...
    if (!unknowns.empty()) {
        MessageDialog dlg(this,
            wxString::Format(_L("There are some unknown or incompatible filaments mapped to generic preset.\nPlease update %s or restart %s to check if there is an update to system presets."), wxString(SLIC3R_APP_FULL_NAME), wxString(SLIC3R_APP_FULL_NAME)) + detail,
            _L("Sync filaments with AMS"), wxOK);
        dlg.ShowModal();
    }
    if (!sync_color_only) {
        ...
        try {
            if (type_3mf) {
                ...
                En3mfType          en_3mf_file_type = En3mfType::From_BBS;
                {
                    ...
                    if (en_3mf_file_type == En3mfType::From_Prusa) {
                        ...
                        if (file_version <= inlong_tag_start_version) {
                            ...
                            if (load_config && config_loaded.empty()) {
                                ...
                                wxString text = _L("The 3MF was created by BambuStudio. Some settings may differ from Inlong Slicer.");
                                log_and_show_3mf_info(text, bambu_project_title);
                            }
                        }
                    }
                    ...
                    // INLONG: legacy feature-filament default migration (1 -> 0) is now handled
                    ...
                    if (load_config && !config_loaded.empty()) {
                        ...
                        if (!validity.empty()) {
                            BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ":" << __LINE__ << " " << boost::format("3MF imported with invalid parameter values:");
                            for (std::map<std::string, std::string>::iterator it=validity.begin(); it!=validity.end(); ++it)
                                BOOST_LOG_TRIVIAL(info) << __FUNCTION__ << ":" << __LINE__ << " " << boost::format("3MF parameter validation issue: %1%: %2%")%it->first %it->second;
                            //
...
void Plater::priv::bring_instance_forward() const
{
    ...
    BOOST_LOG_TRIVIAL(debug) << "Inlong Slicer window going forward";
            ...
            http.on_progress([&percent, &cont, &msg, &filesize, &size_limit](Http::Progress progress, bool& cancel) {
                ...
                .on_error([&msg, &cont, &retry_count, max_retries](std::string body, std::string error, unsigned http_status) {
                    ...
                    if (retry_count == max_retries) {
                        msg = wxString::Format(_L("Importing to %s failed. Please download the file and manually import it."), wxString(SLIC3R_APP_FULL_NAME));
                        cont = false;
                    }
                })
...
void Plater::on_config_change(const DynamicPrintConfig &config)
{
    ...
    for (auto opt_key : diff_keys) {
        ...
        // Inlong: update when *_filament changed
                 ...
                 opt_key == "wall_filament" || opt_key == "sparse_infill_filament" || opt_key == "solid_infill_filament" ||
    ...
    if (type == PrinterWarningType::NOT_CONNECTED) {
        if (device_page) {
            ...
            content = wxString::Format(
                _L("%s can't connect to %s. Please check if the printer is powered on and connected to the network."), wxString(SLIC3R_APP_FULL_NAME), printer_name);
        }
...
bool Plater::refresh_missing_plugin_block(bool* block_toggled)
{
    ...
    update(NotificationType::InlongCloudPluginMissingError, missing_cloud,
           &p->m_cloud_missing_shown_sig,
           _u8L("Inlong Cloud plugins required by the current preset are not installed:"),
           _u8L("Install Plugins"),
           [this, missing_cloud_refs](wxEvtHandler*) { install_missing_cloud_plugins(missing_cloud_refs); return false; });
    // "Find on Inlong Cloud" is only a suggestion: it opens the browser but cannot resolve the missing
    ...
    update(NotificationType::InlongLocalPluginMissingError, missing_local,
           &p->m_local_missing_shown_sig,
           _u8L("Local plugins required by the current preset are missing:"),
           _u8L("Find on Inlong Cloud"),
    ...
    update(NotificationType::InlongPluginInactiveError, inactive,
    ...
    update(NotificationType::InlongPluginCapabilityUnavailableError, broken,
