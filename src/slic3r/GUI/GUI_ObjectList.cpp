...
public:
    ...
    {   // INLONG draw selection background to improve consistency between platforms
        dc.SetBrush(StateColor::darkModeColorFor(wxColour("#F5DAD1")));
    ...
    {   // INLONG draw focus rectangle to improve consistency between platforms
        dc.SetPen(  StateColor::darkModeColorFor(wxColour("#D66C47")));
        dc.DrawRectangle(rect);
    }
    virtual void DrawTreeItemButton(   wxWindow *win, wxDC& dc, const wxRect& rect, int flags = 0) override
    {   // INLONG draw custom triangle to improve consistency between platforms
    ...
    {   // INLONG draw custom text to improve consistency between platforms
...
ObjectList::ObjectList(wxWindow* parent) :
    wxDataViewCtrl(parent, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxDV_MULTIPLE | wxNO_BORDER | wxDV_NO_HEADER) // INLONG: Remove border and header
{
    wxGetApp().UpdateDVCDarkUI(this, true);

#ifdef __linux__
    // Temporary fix for incorrect dark mode application regarding list item's text color.
    // See upstream issue #2086.
...
void ObjectList::show_context_menu(const bool evt_context_menu)
{
    ...
    if (multiple_selection())
    {
        ...
        if (item)
        {
			...
			volume->is_svg() ? plater->svg_part_menu() : // INLONG fixes missing "Edit SVG" item for Add/Negative/Modifier SVG objects in object list
                    plater->part_menu();
            }
            else
...
// INLONG: kept as dead code (not called by any active path). Preserved in #if 0
...
void ObjectList::change_part_type()
{
  ...
  if (selections.size() <= 1) {
    ...
    // INLONG: Fix crash when changing type of svg / text modifier
    wxArrayString names;
    names.Add(_L("Part"));
...
void ObjectList::set_extruder_for_selected_items(const int extruder)
{
    ...
    for (const wxDataViewItem& sel_item : sels)
    {
        ...
        // INLONG: Fix crash when setting filament for instance (item was used uninitialized)
