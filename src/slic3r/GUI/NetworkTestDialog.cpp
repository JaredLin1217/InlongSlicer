...
wxBoxSizer* NetworkTestDialog::create_info_sizer(wxWindow* parent)
{
	...
	text_version_title = new wxStaticText(this, wxID_ANY, wxString(SLIC3R_APP_FULL_NAME) + " " + _L("Version:"), wxDefaultPosition, wxDefaultSize, 0);
...
wxBoxSizer* NetworkTestDialog::create_content_sizer(wxWindow* parent)
{
	...
	btn_link = new Button(this, _L("Test Inlong Slicer (GitHub)"));
    btn_link->SetStyle(ButtonStyle::Regular, ButtonType::Window);
	grid_sizer->Add(btn_link, 0, wxEXPAND | wxALL, 5);

	text_link_title = new wxStaticText(this, wxID_ANY, _L("Test Inlong Slicer (GitHub):"), wxDefaultPosition, wxDefaultSize, 0);
...
void NetworkTestDialog::start_all_job_sequence()
{
	m_sequence_job = new boost::thread([this] {
        ...
        if (m_closing) return;
		start_test_url(TEST_INLONG_JOB, "InlongSlicer(GitHub)", "https://github.com/JaredLin1217/InlongSlicer");
		if (m_closing) return;
...
{
    if (m_in_testing[TEST_INLONG_JOB])
        return;
    test_job[TEST_INLONG_JOB] = new boost::thread([this] {
        start_test_url(TEST_INLONG_JOB, "InlongSlicer(GitHub)", "https://github.com/JaredLin1217/InlongSlicer");
    });
}
...
{
	return wxString(INLONGSLICER_VERSION);
}
