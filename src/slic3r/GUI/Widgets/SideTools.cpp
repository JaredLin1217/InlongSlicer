	...
	SideToolsPanel::SideToolsPanel(wxWindow *parent, wxWindowID id, const wxPoint &pos, const wxSize &size)
{
    ...
    m_none_printing_img = ScalableBitmap(this, "tab_monitor_active", 20); // INLONG match icon size with exact resolution to fix blurry icon
...
void SideToolsPanel::doRender(wxDC &dc)
{
    ...
    if (m_none_printer) {
        dc.SetPen(StateColor::darkModeColorFor(SIDE_TOOLS_BRAND));   // INLONG: Sidebar header background color - Fix for dark mode compability
        dc.SetBrush(StateColor::darkModeColorFor(SIDE_TOOLS_BRAND)); // INLONG: Sidebar header background color - Fix for dark mode compability
        dc.DrawRectangle(0, 0, size.x, size.y);
    ...
    if (((status & (int)MonitorStatus::MONITOR_DISCONNECTED) != 0) || ((status & (int)MonitorStatus::MONITOR_DISCONNECTED_SERVER) != 0)) {
        if ((status & (int)MonitorStatus::MONITOR_DISCONNECTED_SERVER)) {
            ...
            update_connect_err_info(BAMBU_NETWORK_ERR_CONNECTION_TO_PRINTER_FAILED,
                ...
                _L("Please check the network connection of the printer and Inlong Slicer."));
        }
        ...
        m_connection_info->SetBackgroundColor(0xD66C47);
        m_connection_info->SetBorderColor(0xD66C47);
        m_connection_info->Show();
        m_more_button->Hide();
        m_side_error_panel->Hide();
    }
