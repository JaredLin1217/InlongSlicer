...
void AMSMaterialsSetting::create_panel_normal(wxWindow* parent)
{
    ...
    m_readonly_filament->SetBorderColor(StateColor(std::make_pair(0xDBDBDB, (int)StateColor::Focused), std::make_pair(0xD66C47, (int)StateColor::Hovered),
...
void AMSMaterialsSetting::create_panel_kn(wxWindow* parent)
{
    ...
    // Orca: link to the Inlong Slicer pressure-advance wiki (region-agnostic).
...
void AMSMaterialsSetting::set_color(wxColour color)
{
    ...
    m_clr_name->SetLabelText(clr_query->GetFilaColorName(agent ? agent->from_inlong_filament_id(ams_filament_id) : ams_filament_id, fila_color));
}
...
{
    if (show) {
        m_button_confirm->Rescale(); // INLONG re applies size
        m_input_nozzle_max->GetTextCtrl()->SetSize(wxSize(-1, FromDIP(20)));
        m_input_nozzle_min->GetTextCtrl()->SetSize(wxSize(-1, FromDIP(20)));
...
void AMSMaterialsSetting::on_select_filament(wxCommandEvent &evt)
{
    ...
    if ( !this->obj || m_filament_selection < 0) {
        m_input_k_val->Enable(false);
        m_input_n_val->Enable(false);
        m_button_confirm->Disable(); // INLONG No need to change style
        ...
        m_button_confirm->Enable(true);  // INLONG No need to change style
    }
...
void AMSMaterialsSetting::on_dpi_changed(const wxRect &suggested_rect)
{
    ...
    m_button_reset->Rescale(); // INLONG
    m_button_confirm->Rescale(); // INLONG
    m_button_close->Rescale(); // INLONG
    this->Refresh();
}
