    ...
    const bool grayscale/* = false*/, const bool dark_mode/* = false*/, const std::string& new_color /*= ""*/, const float scale_in_center/* = 0*/)
{
    ...
    replaces["\"#0x00AE42\""] = "\"#D66C47\"";
        replaces["\"#00FF00\""] = "\"#E18263\"";
    if (dark_mode) {
        ...
        replaces["\"#D66C47\""] = "\"#A64E33\"";
        replaces["\"#F1F1F1\""] = "\"#36363B\"";
        replaces["#DBDBDB"] = "#4A4A51"; // INLONG border color
        replaces["#F0F0F1"] = "#333337"; // INLONG disabled background color
        replaces["#262E30"] = "#EFEFF0"; // INLONG
    } else {
        replaces["#949494"] = "#7C8282"; // INLONG replace icon line color for light theme
    }

    if (strstr(bitmap_name.c_str(), "toggle_on") != NULL && dark_mode) // INLONG only replace color of toggle button
        replaces["#D66C47"] = "#A64E33";

    if (!new_color.empty())
        replaces["\"#D66C47\""] = "\"" + new_color + "\"";
...
bool BitmapCache::load_from_svg_file_change_color(const std::string &filename, unsigned width, unsigned height, ImTextureID &texture_id, const char *hexColor)
{
    ...
    unsigned int green_color = 0xFF476CD6; // #D66C47
