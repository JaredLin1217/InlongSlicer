...
{
    // Inlong: use 500 x 500 bed size as baseline.
...
std::vector<Polylines> Bed_2D::generate_grid(const ExPolygon& poly, const BoundingBox& bb, const Vec2d& origin, const double& step, const double& scale)
{
    ...
    // INLONG draw grid lines relative to origin
...
void Bed_2D::repaint(const std::vector<Vec2d>& shape)
{
    ...
    dc.SetPen(wxPen(wxColour(encode_color(ColorRGB::X())), 2, wxPENSTYLE_SOLID));  // red // INLONG match axis colors
	auto x_end = Vec2d(origin_px(0) + axes_len, origin_px(1));
	dc.DrawLine(wxPoint(origin_px(0), origin_px(1)), wxPoint(x_end(0), x_end(1)));
	//for (auto angle : { -arrow_angle, arrow_angle }) {  // INLONG dont draw arrows
    ...
    dc.SetPen(wxPen(wxColour(encode_color(ColorRGB::Y())), 2, wxPENSTYLE_SOLID));  // green // INLONG match axis colors
	...
	// draw origin
    dc.SetPen(wxPen(wxColour(encode_color(ColorRGB::Z())), 1, wxPENSTYLE_SOLID));    // INLONG match axis colors
    dc.SetBrush(wxBrush(wxColour(encode_color(ColorRGB::Z())), wxBRUSHSTYLE_SOLID)); // INLONG match axis colors
	...
	const auto origin_label_x = origin_px(0) + 2;                       // INLONG always draw (0,0) text in axes bounding box
    ...
    dc.DrawRectangle(wxPoint(origin_label_x, origin_label_y), extent);  // INLONG draw a background to origin position text to improve readability when overlaps with grid
	dc.DrawText(origin_label, origin_label_x, origin_label_y);

    // INLONG add grid size value as information for large scale beds
