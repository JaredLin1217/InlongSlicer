...
std::string DevAmsTray::get_filament_type()
{
    ...
    if (m_fila_type == "Support") {
        ...
        const std::string printer_filament_id = agent ? agent->from_inlong_filament_id(setting_id) : setting_id;
                    ...
                    if (it->contains("tray"))
                    {
                        ...
                        for (auto tray_it = (*it)["tray"].begin(); tray_it != (*it)["tray"].end(); tray_it++)
                        {
                            ...
                            if (tray_it->contains("tray_info_idx") && tray_it->contains("tray_type"))
                            {
                                ...
                                auto* agent = GUI::wxGetApp().getAgent();
                                const std::string printer_filament_id = agent ? agent->from_inlong_filament_id(curr_tray->setting_id) : curr_tray->setting_id;
