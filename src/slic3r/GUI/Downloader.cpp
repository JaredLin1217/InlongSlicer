...
void Downloader::start_download(const std::string& full_url)
{
    ...
    // Replace PS workaround for "mysterious slash" with a more dynamic approach.
    // Windows seems to have fixed the issue and this provides backwards compatability for those it still affects
	boost::regex re(R"(^(inlongslicer|prusaslicer|bambustudio|cura):\/\/open[\/]?\?file=)", boost::regbase::icase);
    ...
    if (is_bambustudio_open(full_url) || (is_inlong_open(full_url) && is_makerworld_link(full_url)))
