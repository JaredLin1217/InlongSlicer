...
bool Slic3r::is_stringing_prone_filament(const std::string& filament_id, float nozzle_diameter)
{
    ...
    return set->count(agent ? agent->from_inlong_filament_id(filament_id) : filament_id) > 0;
}
...
DevAmsTray MachineObject::parse_vt_tray(json vtray)
{
    ...
    if (vt_tray.hold_count > 0) {
        ...
        if (vtray.contains("tray_info_idx") && vtray.contains("tray_type")) {
            ...
            const std::string printer_filament_id = agent ? agent->from_inlong_filament_id(vt_tray.setting_id) : vt_tray.setting_id;
