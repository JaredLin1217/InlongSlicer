...
{
    // INLONG: Fix dark color rendering. Ensure minimal brightness.
    ...
    for (size_t i = 1; i < moves.size(); ++i) {
        ...
        if (option_type == EOptionType::COUNT || option_type == EOptionType::Travels || option_type == EOptionType::Wipes) {
            if (ret.vertices.empty() || prev.type != curr.type || prev.extrusion_role != curr.extrusion_role
                // Inlong: Split the path when a preview value changes.
                ...
                libvgcode::PathVertex vertex = { convert(prev.position), curr.height, curr.width, curr.feedrate, prev.actual_feedrate,
                    ...
                    /* INLONG: Add Pressure Advance visualization support */ 0.0f, curr.pressure_advance,
