    ...
    , m_mapping_tutorial_popup(AmsTutorialPopup(this))
{
    ...
    rename_editable       = new ScalableBitmap(m_scroll_area, "rename_edit", FromDIP(13)); // INLONG Match edit icon and its size
    rename_editable_light = new ScalableBitmap(m_scroll_area, "rename_edit", FromDIP(13)); // INLONG Match edit icon and its size
    ...
    rename_sizer_h->Add(m_rename_text, 0, wxALIGN_CENTER, 0); // INLONG align text with icon
    rename_sizer_h->Add(m_rename_button, 0, wxALIGN_CENTER| wxLEFT, FromDIP(3)); // INLONG add gap between text and icon
    ...
    m_link_edit_nozzle->SetForegroundColour("#D66C47");
    ...
    m_statictext_finish->SetForegroundColour(wxColour(214, 108, 71));
...
void SelectMachineDialog::show_timelapse_storage_dialog(MachineObject* obj)
{
    ...
    if (show_confirm_btn) {
        ...
        // Use the Inlong accent rather than the reference's hard-coded confirm colour.
        StateColor confirm_bg(std::pair<wxColour, int>(wxColour(214, 108, 71), StateColor::Normal));
    ...
    if (!show_confirm_btn) {
        // Use the Inlong accent rather than the reference's hard-coded confirm colour.
        StateColor cancel_bg(std::pair<wxColour, int>(wxColour(214, 108, 71), StateColor::Normal));
        ...
        if (!plate_data_list[i]->gcode_file.empty()) {
            if (agent)
                for (auto& info : plate_data_list[i]->slice_filaments_info)
                    info.filament_id = agent->to_inlong_filament_id(info.filament_id);
...
void SelectMachineDialog::update_show_status(MachineObject* obj_)
{
    ...
    if (obj_ && m_checkbox_list.count("flow_cali") && m_checkbox_list["flow_cali"]->IsShown()
        ...
        if (!warning_tpu_filaments.empty()) {
            ...
            for (const auto& fila : m_ams_mapping_result) {
                ...
                const std::string printer_filament_id = agent ? agent->from_inlong_filament_id(fila.filament_id) : fila.filament_id;
    ...
    // Inlong: show warning if external filament does not match
...
bool SelectMachineDialog::can_support_pa_auto_cali()
{
    ...
    if (!unsupport_auto_cali_filaments.empty()) {
        ...
        auto iter = std::find_if(m_filaments.begin(), m_filaments.end(),
            ...
            const std::string printer_filament_id = agent ? agent->from_inlong_filament_id(item.filament_id) : item.filament_id;
...
void SelectMachineDialog::Enable_Send_Button(bool en)
{
    if (!en) {
        if (m_button_ensure->IsEnabled()) {
            ...
            // INLONG no need to set colors again
        }
    } else {
        if (!m_button_ensure->IsEnabled()) {
...
void SelectMachineDialog::on_dpi_changed(const wxRect &suggested_rect)
{
    ...
    m_button_ensure->Rescale(); // INLONG
...
void SelectMachineDialog::set_default_normal(const ThumbnailData &data)
{
    ...
    if (wxGetApp().app_config->get("use_inches") == "1") {
        ::sprintf(weight, "%.2f oz", aprint_stats.total_weight * 0.035274); // INLONG remove spacing before text
    } else {
        ::sprintf(weight, "%.2f g", aprint_stats.total_weight); // INLONG remove spacing before text
    }
...
void SelectMachineDialog::set_default_from_sdcard()
{
    ...
    try {
        ...
        char weight[64];
        ::sprintf(weight, "%.2f g", float_weight); // INLONG remove spacing before text
        m_stext_time->SetLabel(time);
        m_stext_weight->SetLabel(weight);
...
void PrintOptionItem::doRender(wxDC& dc)
{
    ...
    for (auto it = m_ops.begin(); it != m_ops.end(); ++it)
    {
        ...
        {
            const wxColour& clr = m_enable ? StateColor::darkModeColorFor("#D66C47") : StateColor::darkModeColorFor(wxColour(144, 144, 144));
            dc.SetPen(wxPen(clr));
