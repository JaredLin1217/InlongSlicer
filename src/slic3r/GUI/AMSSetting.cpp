...
void AMSSettingTypePanel::OnAmsTypeChanged(wxCommandEvent& event)
{
    ...
    if (obj_) {
        if (obj_->is_in_printing() || obj_->is_in_upgrading())  {
            MessageDialog dlg(this, _L("The printer is busy and cannot switch AMS type."), wxString(SLIC3R_APP_FULL_NAME) + _L(" Info"), wxOK | wxICON_INFORMATION);
            dlg.ShowModal();
        ...
        if (ext && ext->HasFilamentInExt()) {
            MessageDialog dlg(this, _L("Please unload all filament before switching."), wxString(SLIC3R_APP_FULL_NAME) + _L(" Info"), wxOK | wxICON_INFORMATION);
        ...
        MessageDialog dlg(this, _L("AMS type switching needs firmware update, taking about 30s. Switch now?"), wxString(SLIC3R_APP_FULL_NAME) + _L(" Info"), wxOK | wxCANCEL | wxICON_INFORMATION);
        dlg.SetButtonLabel(wxID_OK, _L("Confirm"));
...
void AMSSettingArrangeAMSOrder::OnBtnRearrangeClicked(wxCommandEvent& event)
{
    ...
    if (part)  {
        MessageDialog dlg(this, _L("AMS ID will be reset. If you want a specific ID sequence, "
                                   ...
                                   wxString(SLIC3R_APP_FULL_NAME) + _L(" Info"), wxOK | wxCANCEL | wxICON_INFORMATION);
        int rtn = dlg.ShowModal();
        if (rtn == wxID_OK) {
