...
if [ "$build_tool" = 1 ]; then
    ...
    if ! cmake "${build_args[@]}"; then
        echo "ERROR: could not build generate_system_cache — configure the build tree with -DINLONG_TOOLS=ON:" >&2
        echo "       cmake -S \"$repo_root\" -B \"$build_dir\" -DINLONG_TOOLS=ON" >&2
        exit 1
    fi
    tool=$(find_tool)
fi
...
if [ -z "$tool" ]; then
    echo "ERROR: generate_system_cache not found under $build_dir — build with -DINLONG_TOOLS=ON" >&2
    exit 1
fi
