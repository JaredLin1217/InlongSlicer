...
.DESCRIPTION
    ...
    deleted. Failure-path cases use disposable compiler/CMake fixtures rather
    than installed optional toolchains or presumed missing drive letters. Each
...
$fixtures = Join-Path ([IO.Path]::GetTempPath()) ('build_win_test_fixtures-' + [guid]::NewGuid().ToString('N'))
...
# Compiler discovery is tested against disposable files, not the host's optional
# LLVM installation. The stub implements only the two vswhere properties read by
# build_win.bat; no compiler or developer shell is executed by these dry runs.
$vsProgramFiles = Join-Path $fixtures 'Program Files (x86)'
$vsInstaller = Join-Path $vsProgramFiles 'Microsoft Visual Studio\Installer'
$vsInstallation = Join-Path $vsInstaller 'Test Visual Studio'
New-Item -ItemType Directory -Force -Path $vsInstaller | Out-Null
$vswhereSource = Join-Path $fixtures 'vswhere.cs'
Set-Content -LiteralPath $vswhereSource -Encoding ascii -Value @'
using System;
using System.IO;
public class TestVsWhere {
    public static int Main(string[] args) {
        if (Array.IndexOf(args, "catalog_productDisplayVersion") >= 0)
            Console.WriteLine("18.0");
        else if (Array.IndexOf(args, "resolvedInstallationPath") >= 0)
            Console.WriteLine(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Test Visual Studio"));
        return 0;
    }
}
'@
...
& $csc /nologo /target:exe "/out:$(Join-Path $vsInstaller 'vswhere.exe')" $vswhereSource
if ($LASTEXITCODE -ne 0) { throw 'Could not build the disposable vswhere fixture' }
foreach ($hostArch in 'x64', 'ARM64') {
    $bin = Join-Path $vsInstallation "VC\Tools\Llvm\$hostArch\bin"
    New-Item -ItemType Directory -Force -Path $bin | Out-Null
    Copy-Item "$env:SystemRoot\System32\where.exe" (Join-Path $bin 'clang-cl.exe')
}
$standaloneLlvm = Join-Path $fixtures 'Standalone LLVM'
$standaloneBin = Join-Path $standaloneLlvm 'bin'
New-Item -ItemType Directory -Force -Path $standaloneBin | Out-Null
$standaloneClang = Join-Path $standaloneBin 'clang-cl.exe'
Copy-Item "$env:SystemRoot\System32\where.exe" $standaloneClang
$standaloneClangCmake = $standaloneClang.Replace('\', '/')
$failingCmakeDir = Join-Path $fixtures 'failing-cmake'
New-Item -ItemType Directory -Force -Path $failingCmakeDir | Out-Null
$failingCmakeSource = Join-Path $fixtures 'failing-cmake.cs'
$failingCmakeExe = Join-Path $failingCmakeDir 'cmake.exe'
Set-Content -LiteralPath $failingCmakeSource -Encoding ascii -Value @'
public class FailingCMake {
    public static int Main(string[] args) {
        if (args.Length == 1 && args[0] == "--version") {
            System.Console.WriteLine("cmake version 4.0.0 (test fixture)");
            return 0;
        }
        return 1;
    }
...
'@
& $csc /nologo /target:exe "/out:$failingCmakeExe" $failingCmakeSource
if ($LASTEXITCODE -ne 0) { throw 'Could not build the disposable CMake fixture' }
& $failingCmakeExe --build $fixtures
if ($LASTEXITCODE -ne 1) { throw 'The CMake failure fixture must exit 1' }
$missingDeps = Join-Path $fixtures 'missing-deps'
$missingBuild = Join-Path $fixtures 'missing-build'
...
$cases = @(
    ...
    @{ Name = 'the environment section shows what to set'; Args = @('--help'); DryRun = $false
       Contains = @('INLONG_DEPS_CMAKE_ARGS', 'INLONG_SLICER_CMAKE_ARGS', 'INLONG_UPDATER_SIG_KEY', 'NINJA_STATUS',
                    'set INLONG_SLICER_CMAKE_ARGS=-DSLIC3R_BUILD_SANDBOXES=ON', '(PowerShell)', 'debugscript') }
    ...
    @{ Name = '--clang-path names the compiler, quoted for its spaces'; Args = @('-d', '-x', '--clang-path', $standaloneClang)
       Contains = @('-DCMAKE_C_COMPILER="' + $standaloneClangCmake + '"',
                    '-DCMAKE_CXX_COMPILER="' + $standaloneClangCmake + '"') }
    @{ Name = '--clang-path beats the Visual Studio clang'; Args = @('-s', '-x', '--clang-path', $standaloneClang)
       Contains = @('Compiler: ' + $standaloneClangCmake) }
    @{ Name = '--clang-path is a clang request on its own'; Args = @('-d', '-x', '--clang-path', $standaloneClang)
       Contains = @('deps/build-clang') }
    @{ Name = '--clang-path needs Ninja to take effect'; Args = @('-d', '--clang-path', $standaloneClang); ExpectExit = 1
       Contains = @('needs the Ninja generator') }
    # `exist` is true for a directory too, and a directory would reach CMake
    # as the compiler.
    @{ Name = '--clang-path must name the exe, not its folder'; Args = @('-d', '-x', '--clang-path', $standaloneLlvm); ExpectExit = 1
    ...
    @{ Name = '--clang-path contradicting --msvc is rejected'; Args = @('-d', '-x', '--msvc', '--clang-path', $standaloneClang); ExpectExit = 1
    ...
    @{ Name = '-p packs the deps tree with whichever archiver is usable'; Args = @('-d', '-p')
       Match = @('^\+ .*(7z\.exe a|tar\.exe -a -c -f) InlongSlicer_dep_win-\S+\.zip InlongSlicer_dep$') }
    ...
    @{ Name = 'the bundle name carries compiler and deps flavour'; Args = @('-p', '-l', '--config', 'debug', '--arch', 'arm64')
       Contains = @('InlongSlicer_dep_win-ARM64-clang-dbg_') }
    ...
    # install(TARGETS InlongSlicer_app_gui) has no OPTIONAL, so this would fail partway
    ...
    @{ Name = 'naming the executable target is allowed'; Args = @('-s', '-i', '--slicer-target', 'InlongSlicer_app_gui')
       Contains = @('--target install') }
    ...
    @{ Name = '--deps-dir tells the slicer where the deps are'; Args = @('-s', '--deps-dir', 'D:\inlong-deps')
       Contains = @('-DDEP_BUILD_DIR="D:\inlong-deps"') }
    @{ Name = '--deps-dir also redirects the deps build'; Args = @('-d', '--deps-dir', 'D:\inlong-deps')
       Contains = @('cmake -S deps -B "D:\inlong-deps"', 'cmake --build "D:\inlong-deps"')
    ...
    @{ Name = '--deps-dir also redirects the pack'; Args = @('-p', '--deps-dir', 'D:\inlong-deps')
       Contains = @('cd /d "D:\inlong-deps"') }
    ...
    @{ Name = '--build-dir still names the deps tree'; Args = @('-s', '-l', '-x', '--build-dir', 'out/build/x64-clang')
       Match = @('-DDEP_BUILD_DIR="[A-Za-z]:\\.*\\deps\\build-clang"') }
    @{ Name = '--deps-dir wins over the derived tree'; Args = @('-s', '--build-dir', 'out/build/x64-clang', '--deps-dir', 'D:\inlong-deps')
       Contains = @('-DDEP_BUILD_DIR="D:\inlong-deps"') }
    ...
    @{ Name = '-c removes the tree --deps-dir named, not the default one'; Args = @('-d', '-c', '--deps-dir', 'D:\inlong-deps')
       Match = @('^\+ rmdir /S /Q "D:\\inlong-deps"$')
       NotContains = @('\deps\build') }
    ...
    @{ Name = 'INLONG_DEPS_CMAKE_ARGS reaches the deps configure'; Args = @('-d')
       Env = @{ INLONG_DEPS_CMAKE_ARGS = '-DFOO=BAR -DBAZ=QUX' }
       Contains = @('-DFOO=BAR -DBAZ=QUX') }
    @{ Name = 'INLONG_SLICER_CMAKE_ARGS reaches the slicer configure'; Args = @('-s')
       Env = @{ INLONG_SLICER_CMAKE_ARGS = '-DWANTED=1' }
       Contains = @('-DWANTED=1') }
    @{ Name = 'the deps override does not leak into the slicer configure'; Args = @('-s')
       Env = @{ INLONG_DEPS_CMAKE_ARGS = '-DDEPSONLY=1' }
    ...
    @{ Name = 'the help lists the environment overrides'; Args = @('--help'); DryRun = $false
       Contains = @('Environment:', 'INLONG_DEPS_CMAKE_ARGS', 'INLONG_SLICER_CMAKE_ARGS') }
    ...
    # Not a dry run: a cd to a missing directory is a real failure inside a
    ...
    # successful one.
    @{ Name = 'a failure inside a build block reaches the caller'; Args = @('-p', '--deps-dir', $missingDeps)
       DryRun = $false; ExpectExit = 1
    ...
    @{ Name = 'a failure names a retry scoped to the stage that failed'; Args = @('-d', '-s', '--deps-dir', $missingDeps)
       DryRun = $false; ExpectExit = 1
       Contains = @("build_win.bat -d --deps-dir `"$missingDeps`" -c")
       NotContains = @('build_win.bat -ds') }
    # CMake's own failure here is hundreds of lines about package resolution.
    @{ Name = 'a missing dependency tree is named, not left to CMake'; Args = @('-s', '--deps-dir', $missingDeps)
       DryRun = $false; ExpectExit = 1
       Contains = @('Dependencies not found at', "Build them with build_win.bat -d --deps-dir `"$missingDeps`"")
    ...
    @{ Name = 'the missing-deps hint names this toolchain'; Args = @('-s', '-l', '-x', '--deps-dir', $missingDeps)
       DryRun = $false; ExpectExit = 1
       Contains = @("Build them with build_win.bat -d -l -x --deps-dir `"$missingDeps`"")
       NotExists = @($missingDeps) }
    # A dry run configures nothing, so it must not depend on which trees happen
    # to exist on the machine running the suite.
    @{ Name = 'a dry run does not check for the deps tree'; Args = @('-s', '--deps-dir', $missingDeps)
       Contains = @('cmake -B "build"')
       NotContains = @('Dependencies not found') }
    # -d is about to build them, so there is nothing to report yet.
    @{ Name = 'building the deps in the same run skips the check'; Args = @('-d', '-s', '--deps-dir', $missingDeps)
       DryRun = $false; ExpectExit = 1
       NotContains = @('Dependencies not found') }
    # A configure fails before any compiler runs, so -v has nothing to show.
    @{ Name = 'a configure failure is not offered a verbose rebuild'; Args = @('-d', '--deps-dir', $missingDeps)
       DryRun = $false; ExpectExit = 1
    ...
    @{ Name = 'a build failure is'; Args = @('-s', '--no-configure', '--build-dir', $missingBuild)
       DryRun = $false; ExpectExit = 1
       Contains = @('-v     show the failing')
       NotExists = @($missingBuild) }
    ...
    @{ Name = 'a deps retry leaves out the slicer tree'; Args = @('-d', '-s', '--deps-dir', $missingDeps, '--build-dir', $missingBuild)
       DryRun = $false; ExpectExit = 1
       Contains = @("build_win.bat -d --deps-dir `"$missingDeps`" -c")
    ...
    @{ Name = 'a visual studio build names the solution instead'; Args = @('-s')
       Contains = @('Solution      ', 'Open in Visual Studio build\InlongSlicer.sln',
    ...
    @{ Name = 'the binary is named in the build tree it was built in'; Args = @('-s', '-l', '-x')
       Contains = @('build-clang\src\Release\inlong-slicer.exe') }
    @{ Name = 'installing names the installed copy instead'; Args = @('-s', '-l', '-x', '-i')
       Contains = @('build-clang\InlongSlicer\inlong-slicer.exe') }
    ...
    @{ Name = 'a deps retry drops it'; Args = @('-d', '-s', '-i', '--deps-dir', $missingDeps)
       DryRun = $false; ExpectExit = 1
       Contains = @("build_win.bat -d --deps-dir `"$missingDeps`" -c")
       NotContains = @('-d -i') }
    ...
    @{ Name = 'a single-target build does not claim the whole binary'; Args = @('-s', '-l', '-x', '--slicer-target', 'glad')
       ...
       NotContains = @('Run it', 'inlong-slicer.exe', 'Rebuild after edits') }
    # The executable has a target of its own, and naming that one does relink.
    @{ Name = 'naming the executable target still claims the binary'; Args = @('-s', '-l', '-x', '--slicer-target', 'InlongSlicer_app_gui')
       Contains = @('Run it', 'inlong-slicer.exe', 'Rebuild after edits')
       NotContains = @('Target        InlongSlicer_app_gui', 'Relink the binary') }
    ...
    @{ Name = 'the 2026 generator gets the XML solution'; Args = @('-s', '--vs', '2026', '--build-dir', 'D:\tree')
       Contains = @('Solution      D:\tree\InlongSlicer.slnx', 'Open in Visual Studio D:\tree\InlongSlicer.slnx') }
    @{ Name = 'the releases before it get the classic one'; Args = @('-s', '--vs', '2022', '--build-dir', 'D:\tree')
       Contains = @('Solution      D:\tree\InlongSlicer.sln', 'Open in Visual Studio D:\tree\InlongSlicer.sln') }
    @{ Name = 'a solution already on disk wins over the generator'; Args = @('-s', '--vs', '2026', '--build-dir', $slnDir)
       Match = @('^  Solution      .*\\InlongSlicer\.sln$') }
    ...
    @{ Name = 'the VS generator says where the solution is'; Args = @('-s')
       Match = @('^  Solution      .*\\build\\InlongSlicer\.slnx?$') }
    @{ Name = 'the solution path follows the configuration'; Args = @('-s', '--config', 'debug')
       Match = @('^  Solution      .*\\build-dbg\\InlongSlicer\.slnx?$') }
...
function Test-Case {
    ...
    $caseEnvironment = @{ 'ProgramFiles(x86)' = $vsProgramFiles }
    if ($Case.ContainsKey('DryRun') -and -not $Case['DryRun']) {
        $caseEnvironment['PATH'] = "$failingCmakeDir;$env:PATH"
    }
    if ($Case['Env']) {
    ...
    foreach ($path in $Case['NotExists']) {
        $full = if ([IO.Path]::IsPathRooted($path)) { $path } else { Join-Path (Split-Path -Parent $Script) $path }
        if (Test-Path $full) {
            $problems += "created '$path'"
        }
    }
    if ($problems.Count -gt 0) { Write-Host $result.Output }
    return ,$problems
}
