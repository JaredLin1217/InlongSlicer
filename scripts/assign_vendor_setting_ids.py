#!/usr/bin/env python3
"""
Assign deterministic, globally-unique setting_id to InlongSlicer system profiles.
...
Policy (see AGENTS.md "Critical Constraints"):
  * A preset's setting_id is a pure function of its identity:
    ...
    (inlong_extra_profile_check.py). Uniqueness is therefore automatic: two presets
  ...
  * Bambu (BBL) owns the authoritative "G*" id space and is the only reserved vendor:
    ...
    Every other vendor - including InlongFilamentLibrary and Custom - follows the
    deterministic rule.
...
# (inlong_extra_profile_check.py) and kept byte-identical to the C++
