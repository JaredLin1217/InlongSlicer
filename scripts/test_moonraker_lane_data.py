...
OFL_FILAMENT_DIR = os.path.join(REPO_ROOT, "resources", "profiles", "InlongFilamentLibrary", "filament")
...
def load_ofl_presets():
    """Map preset name -> parsed JSON for every InlongFilamentLibrary filament profile."""
    presets = {}
...
def check_ofl_generic_map():
    ...
    InlongFilamentLibrary generic preset carrying a filament_id.
    ...
    """
    print("Checking C++ generic-type map against shipped InlongFilamentLibrary presets...")
    try:
...
def main():
    ...
    parser.add_argument("--check-ofl-map", action="store_true",
                        help="Only run the offline check that the C++ generic-type map "
                             "resolves against shipped InlongFilamentLibrary presets")
