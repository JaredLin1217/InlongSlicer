#!/usr/bin/env python3
"""Tests for scripts/inlong_id_tool.py (stdlib unittest, no external deps).
...
import inlong_id_tool as afi  # noqa: E402
...
class TestUpdateSnapshot(SyntheticTreeCase):
    def test_idempotent_and_deterministic(self):
        ...
        self.assertEqual(snap["ids"]["OGFL99"], {
            "filaments": ["InlongFilamentLibrary/Generic PLA", "VendorA/Generic PLA"],
...
class TestDriftReport(unittest.TestCase):
    def test_reports_both_directions(self):
        ...
        inlong_analysis = afi.analyze_tree(t.profiles)

        lines = ubfi.drift_report(rows, inlong_analysis)

        report = "\n".join(lines)
        self.assertIn("Upstream Only PLA", report)
        self.assertIn("Inlong BBL filaments with no row: 1", report)
