...
if errorlevel 1 (
    echo ERROR: could not build generate_system_cache - configure the build tree with -DINLONG_TOOLS=ON: 1>&2
    echo        cmake -S "%REPO_ROOT%" -B "%BUILD_DIR%" -DINLONG_TOOLS=ON 1>&2
    exit /b 1
)
call :find_tool
if not defined TOOL (
    echo ERROR: generate_system_cache.exe not found under %BUILD_DIR% - build with -DINLONG_TOOLS=ON 1>&2
    exit /b 1
)
