...
from inlong_id_tool import generate_preset_setting_id, check_filament_ids
...
# NOTE: currently Inlong expects compatible_printers to be a defined in every instantiation profile, inheritation is not supported in Profile page
def check_filament_compatible_printers(vendor, vendor_folder):
    ...
    In the InlongFilamentLibrary 'compatible_printers' is optional: a profile without it is generic and
    ...
    for profile in profiles.values():
        ...
        if instantiation and vendor != 'InlongFilamentLibrary':
            try:
...
def check_machine_default_materials(profiles_dir, vendor_name):
    ...
    the vendor's filament library or in the global InlongFilamentLibrary.
    ...
    global_filaments = load_available_filament_profiles(profiles_dir, "InlongFilamentLibrary")
...
# vendor. Every other vendor (incl. InlongFilamentLibrary and Custom) must also match the
# formula.
...
def check_setting_id_uniqueness(profiles_dir):
    """
    Validate setting_id across every vendor (see scripts/inlong_id_tool.py):
      ...
      2. A stored setting_id must equal generate_preset_setting_id(vendor, type, name); a stale
         ...
         "python scripts/inlong_id_tool.py --generate --setting-id".
            ...
            for file_path in base.rglob("*.json"):
                ...
                if "settings_id" in data:
                    ...
                    print_error(
                        ...
                        f'"python scripts/inlong_id_tool.py --generate --setting-id"'
                    )
                ...
                if not sid:
                    ...
                    print_error(
                        ...
                        f'run "python scripts/inlong_id_tool.py --generate --setting-id"'
                    )
                    continue
...
def main():
    ...
    # Global filament_id check (see scripts/inlong_id_tool.py): effective ids
    ...
    if errors_found > 0 or warnings_found > 0 :
        print_warning('Issue(s) found, try `inlong_filament_lib.py --fix` to fix common issues automatically')
