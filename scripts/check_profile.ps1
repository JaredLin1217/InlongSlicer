...
$ValidatorReleaseUrl = 'https://github.com/JaredLin1217/InlongSlicer/releases/download/nightly-builds'
...
if (-not $Validator) { $Validator = $env:INLONG_PROFILE_VALIDATOR }
...
# First locally built InlongSlicer_profile_validator.exe, in the order above. An x86 build, and on
...
function Find-LocalValidator {
    ...
    foreach ($pattern in (Get-ValidatorSearchDirs)) {
        foreach ($dir in @(Resolve-Path -Path $pattern -ErrorAction SilentlyContinue)) {
            $candidate = Join-Path $dir.Path 'InlongSlicer_profile_validator.exe'
...
function Save-NightlyValidator {
    ...
    $exe = Join-Path $WorkDir 'validator\InlongSlicer_profile_validator.exe'
    Save-Asset -Url "$ValidatorReleaseUrl/InlongSlicer_profile_validator_Windows_nightly.exe" -Dest $exe
    return $exe
}
...
$CheckBodies = @{
    ...
    # Every released fixture is a snapshot of user presets saved by that InlongSlicer version; each
    ...
    validate_custom = {
        ...
        foreach ($fixture in $fixtures) {
            ...
            # validator opens only that vendor and InlongFilamentLibrary and skips the other sixty-odd
            ...
            Get-ChildItem -LiteralPath $ProfilesDir -Force |
                Where-Object { -not $Vendor -or -not $_.PSIsContainer -or $_.Name -eq $Vendor -or $_.Name -eq 'InlongFilamentLibrary' } |
...
try {
    ...
    # rather than run for a vendor that ships no printers (the filament-only InlongFilamentLibrary);
