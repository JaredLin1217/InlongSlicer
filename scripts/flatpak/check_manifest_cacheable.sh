#!/bin/bash
# Check a Flatpak manifest for `type: dir` sources at or before the inlong_deps
# module.
#
# Usage: check_manifest_cacheable.sh [manifest]
#        Defaults to io.github.JaredLin1217.InlongSlicer.yml next to this script.
#
# Exits 0 when none are found, 1 when any are, listing them as file:line, and
# 2 when the manifest is missing or has no inlong_deps module.

set -euo pipefail

anchor=inlong_deps
manifest=${1:-$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/io.github.JaredLin1217.InlongSlicer.yml}
