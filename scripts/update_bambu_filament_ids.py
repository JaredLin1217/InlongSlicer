#!/usr/bin/env python3
"""
Generate resources/printers/bambu_filament_ids.json: the map from Inlong's
content-addressed filament_id ("OF" + 6 base62 chars, see inlong_id_tool.py)
...
Inlong's: Inlong's BBL bundle is a fork of Bambu's, tuned and extended
...
Inlong's own filament_id policy does). A row's key is the OF id that product's
(filament_vendor, filament_type, filament) triple mints — the id Inlong carries
...
Orca ships nothing with the same identity for, and a count of Inlong's own BBL
...
from inlong_id_tool import (  # noqa: E402
    BAMBU_MAP_PATH,
    OFL,
    PROFILES_DIR,
    analyze_tree,
...
def drift_report(rows, inlong_analysis):
    """Two triple-identity comparisons between the map just derived and Inlong's
    ...
    same-triple filament in Inlong's BBL bundle (upstream ships it, we ship
    nothing with that identity there — sometimes a genuinely missing product,
    sometimes a renamed one), then a count of Inlong's own BBL filaments that
    matched no BambuStudio row (Inlong-only products, e.g. a name-drifted
    duplicate of one already counted in the first list).
    """
    row_triples = {(r["vendor"], r["type"], r["name"]) for r in rows.values()}

    bbl_filaments = inlong_analysis["vendors"].get("BBL", {})
    ofl_filaments = inlong_analysis["vendors"].get(OFL, {})
    inlong_filaments = {}
    for rec in bbl_filaments.values():
        if not rec["instantiation"]:
            continue
        triple = resolve_triple(rec["name"], bbl_filaments, ofl_filaments)
        inlong_filaments.setdefault(base_name(rec["name"]), triple)

    lines = []
    for triple in sorted(row_triples - set(inlong_filaments.values())):
        lines.append(f'upstream ships {triple[2]!r} ({triple[0]}/{triple[1]}), '
                     "we ship nothing with that identity")
    inlong_only = [name for name, triple in inlong_filaments.items()
                 if triple not in row_triples]
    lines.append(f"Inlong BBL filaments with no row: {len(inlong_only)} Inlong-only product(s)")
    return lines
