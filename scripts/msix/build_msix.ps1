...
#>
param(
    [string]$InstallDir = "build/InlongSlicer",
    [string]$OutputPath = "build/InlongSlicer_Windows_MSIX.msix",
    [ValidateSet("x64", "arm64")]
    [string]$Architecture = "x64",
    [string]$StagingDir = "",
    [switch]$StageOnly,
    [string]$IdentityName = "InlongSlicer.InlongSlicer",
    [string]$Publisher = "CN=38F7EA55-C73B-4072-B3B2-C8E0EA15BB82",
    [string]$PublisherDisplayName = "InlongSlicer"
)
$ErrorActionPreference = 'Stop'

$repoRoot = Split-Path (Split-Path $PSScriptRoot -Parent) -Parent

# MSIX version = MAJOR.MINOR.PATCH.0 from the INLONGSLICER_VERSION semver triplet
# (Store requires the revision field to be 0).
$versionContent = Get-Content (Join-Path $repoRoot 'version.inc') -Raw
if ($versionContent -notmatch 'set\(INLONGSLICER_VERSION "(\d+)\.(\d+)\.(\d+)') {
    throw "Could not parse INLONGSLICER_VERSION from version.inc"
}
...
if (-not (Test-Path (Join-Path $InstallDir 'inlong-slicer.exe'))) {
    throw "inlong-slicer.exe not found in '$InstallDir' - build the install tree first"
}

if ([string]::IsNullOrEmpty($StagingDir)) {
    $StagingDir = Join-Path ([System.IO.Path]::GetTempPath()) 'inlong-msix-staging'
}
