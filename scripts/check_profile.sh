...
VALIDATOR_RELEASE_URL="https://github.com/JaredLin1217/InlongSlicer/releases/download/nightly-builds"
...
Checks (default: all, in this order):
  extra_json_check              scripts/inlong_extra_profile_check.py
...
Options:
  ...
  -v, --vendor NAME    check only this vendor, named after its <Vendor>.json (e.g. "Co Print")
      --validator BIN  InlongSlicer_profile_validator to use; also \$INLONG_PROFILE_VALIDATOR.
...
# First locally built InlongSlicer_profile_validator, in the order above. macOS puts it in an .app
...
find_local_validator() {
    ...
    while IFS= read -r dir; do
        [ -d "${dir}" ] || continue
        for candidate in \
            "${dir}/InlongSlicer_profile_validator" \
            "${dir}/InlongSlicer_profile_validator.exe" \
            "${dir}/InlongSlicer_profile_validator.app/Contents/MacOS/InlongSlicer_profile_validator"; do
...
download_validator() {
    ...
    case "$(uname -s)" in
        Linux*)
            ...
            binary="${dest}/InlongSlicer_profile_validator"
            fetch "${VALIDATOR_RELEASE_URL}/InlongSlicer_profile_validator_Linux_Ubuntu2404_nightly" "${binary}" || return 1
            chmod +x "${binary}" || return 1
            ;;
        Darwin*)
            dmg="${dest}/InlongSlicer_profile_validator.dmg"
            app="${dest}/InlongSlicer_profile_validator.app"
            binary="${app}/Contents/MacOS/InlongSlicer_profile_validator"
            fetch "${VALIDATOR_RELEASE_URL}/InlongSlicer_profile_validator_Mac_universal_nightly.dmg" "${dmg}" || return 1
...
check_extra_json_check() {
    python3 "${REPO_ROOT}/scripts/inlong_extra_profile_check.py" --vendor "${VENDOR}"
}
...
PY
    ...
    while IFS=$'\t' read -r version asset expected_sha256; do
        ...
        if [ -n "${VENDOR}" ]; then
            # -v has the validator open this vendor and InlongFilamentLibrary and skip the rest
            ...
            cp -a "${PROFILES_DIR}"/*.json "${PROFILES_DIR}/${VENDOR}" \
                "${PROFILES_DIR}/InlongFilamentLibrary" "${profile_tree}/" || return 1
...
# than run for a vendor that ships no printers (the filament-only InlongFilamentLibrary); naming the
