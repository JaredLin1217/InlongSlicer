#!/usr/bin/env python3
"""
Assign and validate the deterministic ids of InlongSlicer system profiles: the
...
filament_id policy (see docs/HLSD/filament_id.md):
  * filament_id is a PRODUCT id: one named spool product = one id, shared by all
    ...
    InlongFilamentLibrary (OFL) preset — as long as the id it ends up with is the
...
setting_id policy (see AGENTS.md "Critical Constraints"):
  * setting_id is a PRESET id, a pure function of the preset's identity:
    ...
    the validator (inlong_extra_profile_check.py) imports the rule from here.
...
the vendor map, with InlongFilamentLibrary base-bundle fallback; once a chain enters
...
Run from anywhere:
  python scripts/inlong_id_tool.py --generate         write the ids every profile should carry
  python scripts/inlong_id_tool.py --dry-run          preview that; writes nothing
  python scripts/inlong_id_tool.py --check            validate filament_id state (what CI runs)
  python scripts/inlong_id_tool.py --update-snapshot  re-record the sanctioned filament_id state
...
GENERATE_CMD = "python scripts/inlong_id_tool.py --generate"
UPDATE_HINT = 'run "python scripts/inlong_id_tool.py --update-snapshot" and commit the diff for maintainer review'
...
# Same output helpers/format as inlong_extra_profile_check.py (not imported from
...
def list_profile_dirs(profiles_dir):
    ...
    What the setting_id pass walks, and what inlong_extra_profile_check.py walks:
...
# ---------------------------------------------------------------------------
# Checks (imported and called tree-wide by inlong_extra_profile_check.py)
# ---------------------------------------------------------------------------
...
def generate_setting_ids(profiles_dir=PROFILES_DIR, vendors=None, dry_run=False,
    ...
    for vendor in names:
        for path, type_name in iter_profile_files(os.path.join(profiles_dir, vendor)):
            ...
            # Strictly "true", exactly as inlong_extra_profile_check.py tests it:
...
EXAMPLES = """\
examples:
  inlong_id_tool.py --generate
      give every profile the id its identity mints, in every vendor bundle
  inlong_id_tool.py --dry-run
      preview exactly that; writes nothing
  inlong_id_tool.py --generate --setting-id
      setting_id only (filament, process and machine presets)
  inlong_id_tool.py --generate --filament-id --vendor Creality --vendor Elegoo
      filament_id only, and only in those two bundles
  inlong_id_tool.py --check
      validate filament_id state against the snapshot (the filament_id half
      of scripts/inlong_extra_profile_check.py, which is what CI runs)
  inlong_id_tool.py --update-snapshot
...
def build_parser():
    parser = argparse.ArgumentParser(
        prog="inlong_id_tool.py", allow_abbrev=False,
        formatter_class=argparse.RawDescriptionHelpFormatter,
        description="Assign and validate the deterministic ids of InlongSlicer system\n"
...
def main(argv=None):
    ...
    if filament_files and not args.dry_run:
        ...
        print_warning('now run "python scripts/inlong_id_tool.py --update-snapshot" '
