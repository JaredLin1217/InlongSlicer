...
# Inlong Slicer Flatpak Build Script
# This script builds and packages Inlong Slicer as a Flatpak package locally
...
show_help() {
    ...
    echo ""
    echo "Build Inlong Slicer as a Flatpak package"
    echo ""
    echo "Options:"
...
echo -e "${BLUE}Inlong Slicer Flatpak Build Script${NC}"
echo -e "${BLUE}================================${NC}"
...
if [[ -f "version.inc" ]]; then
    VER_PURE=$(grep 'set(INLONGSLICER_VERSION' version.inc | cut -d '"' -f2)
    VER="V$VER_PURE"
    DATE=$(date +'%Y%m%d')
...
if [[ ! -f "./scripts/flatpak/io.github.JaredLin1217.InlongSlicer.yml" ]]; then
    echo -e "${RED}Error: Flatpak manifest not found at scripts/flatpak/io.github.JaredLin1217.InlongSlicer.yml${NC}"
    exit 1
fi
...
BUNDLE_NAME="InlongSlicer-Linux-flatpak_${VER}_${ARCH}.flatpak"
...
MANIFEST="scripts/flatpak/io.github.JaredLin1217.InlongSlicer.yml"
if [[ "$NO_DEBUGINFO" == true ]]; then
    MANIFEST="scripts/flatpak/io.github.JaredLin1217.InlongSlicer.no-debug.yml"
    sed '/^build-options:/a\  no-debuginfo: true\n  strip: true' \
        scripts/flatpak/io.github.JaredLin1217.InlongSlicer.yml > "$MANIFEST"
...
if ! flatpak-builder \
    ...
    rm -f "scripts/flatpak/io.github.JaredLin1217.InlongSlicer.no-debug.yml"
    exit 1
fi
...
if ! flatpak build-bundle \
    "$BUILD_DIR/repo" \
    "$BUNDLE_NAME" \
    io.github.JaredLin1217.InlongSlicer \
    --arch="$ARCH"; then
...
echo ""
echo -e "${BLUE}To run Inlong Slicer:${NC}"
echo -e "flatpak run io.github.JaredLin1217.InlongSlicer"
echo ""
echo -e "${BLUE}To uninstall:${NC}"
echo -e "flatpak uninstall --user io.github.JaredLin1217.InlongSlicer"
echo ""
if [[ "$FORCE_CLEAN" != true ]]; then
