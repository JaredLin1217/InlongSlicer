...
while getopts ":dpa:snt:xbc:i:j:Tuh" opt; do
  case "${opt}" in
    ...
    h ) echo "Usage: ./build_release_macos.sh [-d]"
        ...
        echo "   -T: Build and run tests (set INLONG_TESTS_BUILD_ONLY=1 to build without running)"
        exit 0
        ;;
    * )
        ;;
...
function build_deps() {
    ...
    for _ARCH in x86_64 arm64; do
        ...
        if [ "$ARCH" == "universal" ] || [ "$ARCH" == "$_ARCH" ]; then
            ...
            DEPS="$DEPS_BUILD_DIR/InlongSlicer_dep"
...
function pack_deps() {
    ...
    (
        set -x
        cd "$DEPS_DIR"
        tar -zcvf "InlongSlicer_dep_mac_${ARCH}_$(date +"%Y%m%d").tar.gz" "build"
    )
}
...
function verify_python_runtime() {
    ...
    local bad
    bad=$(otool -arch all -L "$pybin" "$app/Contents/MacOS/InlongSlicer" | grep "libpython" | grep -v "@rpath/" || true)
    if [ -n "$bad" ]; then
    ...
    if ! otool -arch all -l "$app/Contents/MacOS/InlongSlicer" | grep -q "path @executable_path/python/lib "; then
        echo "ERROR: InlongSlicer lacks the @executable_path/python/lib rpath (relocation regression)" >&2
        exit 1
    fi
...
function build_slicer() {
    ...
    for _ARCH in x86_64 arm64; do
        ...
        if [ "$ARCH" == "universal" ] || [ "$ARCH" == "$_ARCH" ]; then
            ...
            if [ "1." != "$BUILD_ONLY". ]; then
                cmake "${PROJECT_DIR}" \
                    ...
                    -DINLONG_TOOLS=ON \
                    ${INLONG_UPDATER_SIG_KEY:+-DINLONG_UPDATER_SIG_KEY="$INLONG_UPDATER_SIG_KEY"} \
        ...
        # -T also runs the tests; INLONG_TESTS_BUILD_ONLY=1 builds them without
        # running, so CI can build here and run them in a dedicated job.
        if [ "1." == "$BUILD_TESTS". ] && [ "1." != "$INLONG_TESTS_BUILD_ONLY". ]; then
        ...
        (
            cd "$PROJECT_BUILD_DIR"
            mkdir -p InlongSlicer
            cd InlongSlicer
            # remove previously built app
            rm -rf ./InlongSlicer.app
            # fully copy newly built app
            cp -pR "../src$BUILD_DIR_CONFIG_SUBDIR/InlongSlicer.app" ./InlongSlicer.app
            # fix resources
            resources_path=$(readlink ./InlongSlicer.app/Contents/Resources)
            rm ./InlongSlicer.app/Contents/Resources
            cp -R "$resources_path" ./InlongSlicer.app/Contents/Resources
            relocate_python_runtime ./InlongSlicer.app
            ...
            if [ -f "../src$BUILD_DIR_CONFIG_SUBDIR/InlongSlicer_profile_validator.app/Contents/MacOS/InlongSlicer_profile_validator" ]; then
                ...
                rm -rf ./InlongSlicer_profile_validator.app
                cp -pR "../src$BUILD_DIR_CONFIG_SUBDIR/InlongSlicer_profile_validator.app" ./InlongSlicer_profile_validator.app
                # delete .DS_Store file
                find ./InlongSlicer_profile_validator.app/ -name '.DS_Store' -delete
                verify_python_runtime ./InlongSlicer_profile_validator.app
            fi
        )

        # extract version
        # export ver=$(grep '^#define INLONGSLICER_VERSION' ../src/libslic3r/libslic3r_version.h | cut -d ' ' -f3)
        # ver="_V${ver//\"}"
        # echo $PWD
        # if [ "1." != "$NIGHTLY_BUILD". ];
        # then
        ...
        # zip -FSr InlongSlicer${ver}_Mac_${_ARCH}.zip InlongSlicer.app
...
function build_universal() {
    ...
    PROJECT_BUILD_DIR="$PROJECT_DIR/build/$ARCH"
    ARM64_APP="$PROJECT_DIR/build/arm64/InlongSlicer/InlongSlicer.app"
    X86_64_APP="$PROJECT_DIR/build/x86_64/InlongSlicer/InlongSlicer.app"

    mkdir -p "$PROJECT_BUILD_DIR/InlongSlicer"
    UNIVERSAL_APP="$PROJECT_BUILD_DIR/InlongSlicer/InlongSlicer.app"
    rm -rf "$UNIVERSAL_APP"
    cp -R "$ARM64_APP" "$UNIVERSAL_APP"

    echo "Creating universal binaries for InlongSlicer.app..."
    lipo_dir "$UNIVERSAL_APP" "$X86_64_APP"
    echo "Universal InlongSlicer.app created at $UNIVERSAL_APP"
    echo "Universal InlongSlicer.app created at $UNIVERSAL_APP"
    verify_python_runtime "$UNIVERSAL_APP"

    # Create universal binary for profile validator if it exists
    ARM64_VALIDATOR="$PROJECT_DIR/build/arm64/InlongSlicer/InlongSlicer_profile_validator.app"
    X86_64_VALIDATOR="$PROJECT_DIR/build/x86_64/InlongSlicer/InlongSlicer_profile_validator.app"
    if [ -d "$ARM64_VALIDATOR" ] && [ -d "$X86_64_VALIDATOR" ]; then
        echo "Creating universal binaries for InlongSlicer_profile_validator.app..."
        UNIVERSAL_VALIDATOR_APP="$PROJECT_BUILD_DIR/InlongSlicer/InlongSlicer_profile_validator.app"
        ...
        echo "Universal InlongSlicer_profile_validator.app created at $UNIVERSAL_VALIDATOR_APP"
    fi
}
