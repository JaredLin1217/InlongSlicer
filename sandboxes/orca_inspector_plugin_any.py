...
Run it from the Plugins dialog. It opens a NON-MODAL window (InlongSlicer stays
...
# The bundle also exposes sla_prints/sla_materials, omitted as InlongSlicer has no SLA.
COLLECTIONS = [
    ("printers", "Printer"),
    ("filaments", "Filament"),
    ("prints", "Process"),
...
def assemble_dict(instance):
    ...
    getattr-guarded so the panel still runs on an older InlongSlicer build."""
...
function renderInstance(o, i) {
  ...
  const asmRows = !a.available
    ? [['assemble', '<span class="muted">needs a newer InlongSlicer build</span>']]
